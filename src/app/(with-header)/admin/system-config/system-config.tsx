...
export const SystemConfig = ({ config: initialConfig }: SystemConfigProps) => {
  ...
  const setConfigKey = <T extends keyof GlobalConfig>(
    key: T,
    val: GlobalConfig[T],
  ) => {
    setSaved(false)
  ...
  return (
    <main className="flex flex-col max-w-3xl w-full mx-auto">
      ...
      <label className="p-3 w-full flex flex-wrap items-center text-sm md:text-base">
        ...
        <span className="w-full mt-2 text-xs sm:text-sm text-gray-500">
          Normally, only user moderators can add cards to users. By enabling
          this, normal users can add cards.
        </span>
      </label>
      <Divider className="bg-divider/5" />
      <label
        className={`p-3 w-full flex flex-wrap items-center text-sm md:text-base ${config.allow_user_add_card ? '' : 'text-gray-500'}`}
      >
        ...
        <Input
          ...
          onValueChange={(v) =>
            setConfigKey('user_max_card', !v || +v < 1 ? null : +v)
          }
        />
        <span className="w-full mt-2 text-xs sm:text-sm text-gray-500">
          If &ldquo;Allow users to add cards&rdquo; is enabled, this controls
          the max card count per user. Note that user moderators can exceed this
          count.
        </span>
      </label>
      <Divider className="bg-divider/5" />
      ...
      <label className="p-3 w-full flex flex-wrap items-center text-sm md:text-base">
        ...
        <Select
          ...
          selectedKeys={
            new Set(
              [UserPermissions.USER, ...USER_PERMISSION_NAMES.keys()]
        ...
        >
          {[
            [UserPermissions.USER, { title: 'All Users' }] as const,
            ...USER_PERMISSION_NAMES,
          ].map(([permission, { title }]) => (
            <SelectItem key={permission?.toString()}>{title}</SelectItem>
          ))}
        </Select>
        <span className="w-full mt-2 text-xs sm:text-sm text-gray-500">
          Allow these user roles to equip userbox items that they have not
          earned.
        </span>
      </label>
    </main>
