  ...
  return (
    <main className="max-w-5xl mx-auto w-full">
        ...
        <Tooltip content="Create new user">
          <Button
            ...
            onPress={() =>
              promptAccessCode(
                prompt,
                'Enter an access code to create this user',
                (code) => {
                  createUserWithAccessCode(code).then((res) => {
      ...
      >
        {users.map((userEntry) => (
          <AccordionItem
            key={userEntry.uuid ?? userEntry.id}
            id={userEntry.uuid ?? undefined}
            indicator={({ isOpen }) => (
              <Tooltip content="Show cards">
                <div className="flex items-center">
                  <CreditCardIcon className="h-6 w-6 mr-1" />
                  <ChevronDownIcon
                    className={`h-4 transition ${isOpen ? 'rotate-180' : ''}`}
                  />
                </div>
              </Tooltip>
            )}
            ...
            title={
              <header className="w-full flex items-center flex-wrap gap-y-1">
                {!hasPermission(
                  ...
                  UserPermissions.SYSADMIN,
                ) &&
                  userEntry.id !== user?.id && (
                    <Tooltip content="Delete user">
                      <div
                        className="mr-1.5 p-1.5 rounded-lg transition bg-danger hover:brightness-90"
                        onClick={() => {
                          confirm(
                            <span>
                              Do you want to delete this user? This will remove
                              all user data including scores. <br />
                            ...
                            () => {
                              confirm(
                                ...
                                () => {
                                  deleteUser(userEntry.id).then((res) => {
                                    if (res.error) return setError(res.message)
                                    setUsers((u) =>
                                      u.filter((u) => u.id !== userEntry.id),
                                    )
                                  })
                                },
                              )
                ...
                {hasPermission(user?.permissions, UserPermissions.OWNER) && (
                  <Tooltip content="Edit permissions">
                    <div
                      className="mr-1.5 p-1.5 rounded-lg transition bg-default hover:brightness-90"
                ...
                {userEntry.username ? (
                  <>
                    <Link
                      href={`/user/${userEntry.uuid}`}
                      className="text-white font-semibold transition hover:text-secondary"
                    >
                      {userEntry.username}
                    </Link>
                ...
                {[...USER_PERMISSION_NAMES]
                  .filter(
                    ([permission]) =>
                      userEntry.permissions! & (1 << permission),
                  )
                  .map(([permission]) => (
                    <PermissionIcon
                ...
                {userEntry.created_date && (
                  <time
                    ...
                    dateTime={userEntry.created_date.toISOString()}
                  >
                    <span className="font-semibold text-sm">Created </span>
                    {userEntry.created_date.toLocaleTimeString(
                      undefined,
                      FORMAT,
                    )}
                  </time>
                )}
                {userEntry.last_login_date && (
                  <time
                    className="text-xs mr-4"
                    dateTime={userEntry.last_login_date.toISOString()}
                  >
                    <span className="font-semibold text-sm">Last Login: </span>
                    {userEntry.last_login_date.toLocaleTimeString(
                      undefined,
                      FORMAT,
                    )}
                  </time>
              ...
              <Tooltip content="Add new card to this user">
                <Button
                  ...
                  onPress={() =>
                    promptAccessCode(
                      prompt,
                      'Enter an access code to add',
                      (code) =>
                        adminAddCardToUser(userEntry.id, code).then((res) => {
                          if (res.error) return setError(res.message)
                          setUsers(res.data)
                        }),
                    )
