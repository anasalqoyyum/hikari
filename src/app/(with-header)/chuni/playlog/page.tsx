...
export type PlaylogFilterState = {
  [K in ArrayIndices<
    typeof FILTERERS
  > as (typeof FILTERERS)[K]['name']]: (typeof FILTERERS)[K]['value']
}
...
const ChuniPlaylogGrid = ({ items }: { items: ChuniPlaylog['data'] }) => {
  ...
  return (
    <WindowScrollerGrid rowSize={rowSize} colSize={colSize} items={items}>
      {(item) => (
        <div className="p-1 w-full h-full max-w-full">
          <ChuniPlaylogCard
            playlog={item}
            showDetails
...
export default function ChuniPlaylogList() {
  ...
  return (
    ...
    >
      {(_, d) => (
        <div className="w-full max-w-full flex-grow my-2">
