...
const searcher = (
  query: string,
  data: ChuniMusicListProps['music'][number],
) => {
  return (
...
const MusicGrid = ({
  ...
  const [scoreCalculator, setScoreCalculator] = useState<ChuniMusic | null>(
    null,
  )
                ...
                >
                  <div className="aspect-square w-full p-[0.2rem] relative">
                    ...
                    {item.rating && !item.worldsEndTag && (
                      <div
                        className={`${size === 'lg' ? 'text-2xl' : ''} absolute bottom-0.5 left-0.5 bg-gray-200/60 backdrop-blur-sm moz-no-backdrop-blur px-0.5 rounded`}
                      >
                        <ChuniRating
                    ...
                    <ChuniLevelBadge
                      className={`${size === 'lg' ? 'h-14' : 'w-14'} absolute bottom-px right-px`}
                      music={item}
                    />
                    ...
                    <Tooltip content="Score Calculator">
                      <Button
                        isIconOnly
                        className="absolute top-0 right-0 pt-0.5 bg-gray-600/25"
                        size={size === 'xs' ? 'sm' : 'md'}
                        variant="flat"
                        radius="full"
                    ...
                    {!!user?.chuni && (
                      <Button
                        isIconOnly
                        className={`absolute top-0 left-0 pt-1 bg-gray-600/25 ${item.favorite ? 'text-red-500' : ''}`}
                        size={size === 'xs' ? 'sm' : 'md'}
                        variant="flat"
                        radius="full"
                        onPress={() => {
                          ...
                          setMusicList(
                            fullMusicList.map((m) => {
                              if (m.songId !== item.songId) return m
                          ...
                          ;(item.favorite
                            ...
                            .then((res) => {
                              if (res?.error) {
                                ...
                                return setError(
                                  `Failed to set favorite: ${res.message}`,
                                )
                              }
                            })
                      ...
                      >
                        {item.favorite ? (
                          <SolidHeartIcon className="w-3/4" />
                        ) : (
                          <OutlineHeartIcon className="w-3/4" />
                        )}
                      </Button>
                    )}
                  </div>
                  <div className="px-0.5 mb-1 flex">
                    {size === 'lg' && (
                      <div className="h-full w-1/3 mr-0.5">
                        {item.isSuccess ? (
                          <ChuniLampSuccessBadge success={item.isSuccess} />
                        ) : null}
                      </div>
                    )}

                    <div
                      className={`h-full ${size === 'lg' ? 'w-1/3' : 'w-1/2'}`}
                    >
                      {item.scoreRank !== null && (
                        <ChuniScoreBadge
                          variant={getVariantFromRank(item.scoreRank)}
                          className="h-full"
                        >
                    ...
                    <div
                      className={`h-full ml-0.5 ${size === 'lg' ? 'w-1/3' : 'w-1/2'}`}
                    >
                      <ChuniLampComboBadge {...item} />
                    </div>
                  </div>
                  <Link
                    href={`/chuni/music/${item.songId}`}
                  ...
                  >
                    <Ticker hoverOnly noDelay>
                      {item.title}
                    </Ticker>
                  </Link>
                  <Ticker
                    className={`${size === 'lg' ? 'text-medium mb-1.5' : 'text-xs mb-0.5'} text-center px-1 drop-shadow-2xl text-white`}
                    hoverOnly
                    noDelay
                  >
                    {item.artist}
...
export const ChuniMusicList = ({ music, topRating }: ChuniMusicListProps) => {
  ...
  return (
    ...
    >
      {(displayMode, data) => (
        <div className="w-full flex-grow my-2">
          <MusicGrid
            music={data}
            size={DISPLAY_IDS[displayMode as keyof typeof DISPLAY_IDS]}
            fullMusicList={localMusic}
            setMusicList={setLocalMusic}
            topRating={topRating}
          />
        </div>
      )}
