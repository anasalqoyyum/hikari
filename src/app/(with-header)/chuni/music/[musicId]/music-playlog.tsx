...
import {
  ChuniLampComboBadge,
  ChuniLampSuccessBadge,
  ChuniScoreBadge,
  ChuniScoreRankBadge,
  getVariantFromRank,
} from '@/components/chuni/score-badge'
import { ChuniPlaylogCard } from '@/components/chuni/playlog-card'
import { useState } from 'react'
import { CalculatorIcon } from '@heroicons/react/24/outline'
import { Tooltip } from '@nextui-org/tooltip'
import {
  ChuniScoreCalculator,
  ChuniScoreCalculatorProps,
} from '@/components/chuni/score-calculator'
...
export const ChuniMusicPlaylog = ({
  music,
  playlog,
  rating,
  ...
  const [scoreCalculator, setScoreCalculator] =
    useState<ChuniScoreCalculatorProps['music']>(null)
  ...
  return (
    <div className="flex flex-col w-full px-1 sm:px-0">
      <ChuniScoreCalculator
        music={scoreCalculator}
        topRating={rating}
        onClose={() => setScoreCalculator(null)}
      />
      <Accordion selectionMode="multiple" selectedKeys={selected}>
        {difficulties.map((data, i) => {
          ...
          return (
            <AccordionItem
              key={i.toString()}
              classNames={{ trigger: 'py-0 my-2' }}
              onClick={() => {
                const key = i.toString()
                setSelected((s) =>
                  s.has(key)
                    ? new Set([...s].filter((k) => k !== key))
                    : new Set([...s, key]),
                )
              }}
              title={
                <div className="flex flex-row flex-wrap items-center gap-y-1.5 -mr-3">
                  <div
                    className={`flex items-center gap-2 flex-wrap lg:flex-grow ${data.playlog.length ? 'cursor-pointer w-full lg:w-auto' : 'flex-grow'}`}
                  >
                    ...
                    {!data.playlog.length && (
                      <div className="text-right italic text-gray-500 flex-grow">
                        No Play History
                      </div>
                    )}
                    {data.rating && data.chartId !== 5 ? (
                      <ChuniRating
                    ...
                    {data.scoreMax ? (
                      <div className="ml-2 text-center flex-grow sm:flex-grow-0 max-sm:text-sm">
                        <span className="font-semibold">High Score: </span>
                        {data.scoreMax.toLocaleString()}
                      </div>
                    ) : null}
                    {data.maxComboCount ? (
                      <div className="ml-2 text-center flex-grow sm:flex-grow-0 max-sm:text-sm">
                        <span className="font-semibold">Max Combo: </span>
                        {data.maxComboCount.toLocaleString()}
                      </div>
                    ) : null}
                  </div>
                  {badges.length ? (
                    <div
                      className={`flex-grow items-center lg:flex-grow-0 ml-auto mr-auto sm:ml-0 lg:ml-auto lg:mr-0 flex gap-0.5 flex-wrap justify-center sm:justify-start ${data.playlog.length ? 'cursor-pointer' : ''}`}
                    >
                      {badges}
                    </div>
                  ) : null}
                  {data.chartId !== 5 ? (
                    <Tooltip content="Score calculator">
                      <div
                        className="ml-2 text-gray-400 rounded-full w-10 h-10 flex items-center justify-center transition hover:bg-default"
                        onClick={(e) => {
                          e.stopPropagation()
            ...
            >
              <div className="flex flex-wrap gap-x-4 gap-y-2 mb-3 justify-center sm:justify-end max-sm:text-xs">
                <span className="mr-auto max-sm:w-full text-center">
                  <span className="font-semibold">Chart designer:</span>{' '}
                  {data.chartDesigner}
                </span>
                {!!data.tapJudgeCount && (
                  <span>
                ...
                {!!data.flickJudgeCount && (
                  <span>
                    <span className="font-semibold">Flick:</span>{' '}
                    {data.flickJudgeCount}
                  </span>
                )}
                {!!data.holdJudgeCount && (
                ...
                {!!data.slideJudgeCount && (
                  <span>
                    <span className="font-semibold">Slide:</span>{' '}
                    {data.slideJudgeCount}
                  </span>
                )}
                {!!data.airJudgeCount && (
