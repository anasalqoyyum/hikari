...
export default async function ChuniDashboard() {
  ...
  if (!user.chuni) return <ChuniNoProfile />
    ...
    <div className="flex h-full flex-col md:flex-row">
      ...
      <div className="flex flex-col h-full flex-grow">
        <div className="flex flex-wrap-reverse">
          <div className="text-xl font-semibold mt-auto pt-3 hidden md:block pr-3">
            Recent Plays
          </div>
          <ChuniNameplate
            className="hidden md:block max-w-[50rem] w-full ml-auto"
            profile={profile}
          />
        </div>
        <div className="text-lg font-semibold px-4 pt-4 border-t border-gray-500 md:hidden">
          Recent Plays
        </div>
        ...
        <div className="w-full mb-3 px-2">
          <Link href="/chuni/playlog" className="w-full flex justify-center">
            <Button className="w-full sm:w-96">View More</Button>
          </Link>
        </div>
      </div>
    </div>
