...
type EquippedItem = {
  [K in keyof RequiredUserbox]: RequiredUserbox[K][number] | null
}
...
export const ChuniUserbox = ({ profile, userboxItems }: ChuniUserboxProps) => {
  const initialEquipped = useRef(
    Object.fromEntries(
      Object.entries(ITEM_KEYS).map(([key, profileKey]) => [
        key,
        userboxItems[key as keyof RequiredUserbox].find(
          (i) => i.id === profile?.[profileKey],
        ),
      ]),
    ) as EquippedItem,
  )
  const [equipped, setEquipped] = useState<EquippedItem>(
    initialEquipped.current,
  )
  const [saved, setSaved] = useState<SavedItem>({
    ...Object.fromEntries(Object.keys(ITEM_KEYS).map((k) => [k, true])),
    username: true,
  } as any)
  const [playingVoice, setPlayingVoice] = useState(false)
  const [selectedLine, setSelectedLine] = useState(new Set(['0035']))
  const [playPreviews, _setPlayPreviews] = useState(true)
  const [selectingVoice, setSelectingVoice] = useState<
    EquippedItem['systemVoice'] | null
  >(null)
  ...
  useEffect(() => {
    _setPlayPreviews(
      !!(localStorage.getItem('chuni-userbox-play-previews') ?? 1),
    )
  }, [])

  const equipItem = <K extends keyof RequiredUserbox>(
    k: K,
    item: RequiredUserbox[K][number] | undefined | null,
  ) => {
  ...
  const reset = <K extends keyof SavedItem>(...items: K[]) => {
    ...
    setEquipped((e) => ({
      ...e,
      ...Object.fromEntries(
        Object.entries(initialEquipped.current).filter(([k]) =>
          items.includes(k as any),
        ),
      ),
    }))
  ...
  const save = <K extends keyof SavedItem>(...items: K[]) => {
    if (!items.length) items = [...(Object.keys(ITEM_KEYS) as any), 'username']

    const update: Partial<ProfileUpdate> = Object.fromEntries(
      (Object.entries(equipped) as Entries<typeof equipped>)
        .filter(([k]) => items.includes(k as any))
        .map(([k, v]) => [ITEM_KEYS[k], v?.id]),
    )

    if ((items as string[]).includes('username')) update.userName = username
    ...
    updateProfile(update).then(({ error, message }) => {
      if (!error) return
  ...
  const voicePreview = (
    <div className="flex rounded-xl overflow-hidden flex-grow">
      <Select
        label="Preview Voice Line"
        size="sm"
        radius="none"
        className="overflow-hidden min-w-56"
        isDisabled={!playPreviews}
        selectedKeys={selectedLine}
        onSelectionChange={(s) => {
          if (typeof s === 'string' || !s.size) return
          setSelectedLine(s as any)
          play(
            getAudioUrl(
              `chuni/system-voice/${selectingVoice?.cuePath ?? equipped.systemVoice?.cuePath}_${[...s][0]}`,
            ),
          )
        }}
      >
        {CHUNI_VOICE_LINES.map(([line, id]) => (
          <SelectItem key={id}>{line}</SelectItem>
        ))}
      </Select>
      <Button
        isIconOnly
        ...
        onPress={() =>
          playingVoice
            ...
            : play(
                getAudioUrl(
                  `chuni/system-voice/${selectingVoice?.cuePath ?? equipped.systemVoice?.cuePath}_${[...selectedLine][0]}`,
                ),
              )
        }
      >
  ...
  const renderItem = (
    item: { name: string | undefined | null },
    image: string,
    textClass = '',
    containerClass = '',
  ) => (
    <div
      className={`w-full h-full flex flex-col border border-gray-500 rounded-2xl shadow-inner ${containerClass}`}
    >
      <Image
        width={100}
        height={100}
        alt={item.name ?? ''}
        className={`w-full h-auto ${textClass}`}
        src={image}
      />
    ...
    <div className="flex justify-center w-full">
      <div className="grid grid-cols-12 justify-items-center max-w-[50rem] xl:max-w-[100rem] gap-2 flex-grow relative">
            ...
            {(!saved.namePlate || !saved.trophy || !saved.username) && (
              <>
                <Button
                  ...
                  color="danger"
                  variant="light"
                  onPress={() => reset('namePlate', 'trophy', 'username')}
                >
                  Reset
                </Button>
                <Button
                  className="ml-2"
                  color="primary"
                  onPress={() => save('namePlate', 'trophy', 'username')}
                >
                  Save
                </Button>
              </>
          ...
          <div className="flex flex-col items-center h-full w-full xl:max-w-none sm:px-2 sm:pb-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 w-full px-2 sm:px-1">
              <SelectModalButton
                ...
                colSize={500}
                gap={6}
                items={userboxItems.namePlate}
                modalId="nameplate"
                itemId="id"
                renderItem={(n) =>
                  renderItem(
                    n,
                    getImageUrl(`chuni/name-plate/${n.imagePath}`),
                    'w-full sm:text-lg',
                    'px-2 pb-1',
                  )
                }
                selectedItem={equipped.namePlate}
                onSelected={(i) => equipItem('namePlate', i)}
              >
                Change Nameplate
              </SelectModalButton>
              <SelectModalButton
                ...
                modalSize="2xl"
                rowSize={66}
                items={userboxItems.trophy}
                modalId="trophy"
                itemId="id"
                renderItem={(n) => (
                  <ChuniTrophy rarity={n.rareType} name={n.name} />
                )}
                selectedItem={equipped.trophy}
                onSelected={(i) => equipItem('trophy', i)}
              >
                Change Trophy
              </SelectModalButton>
        ...
        <div className="col-span-full xl:col-span-5 flex flex-col w-full rounded-lg sm:shadow-inner sm:bg-content1">
          ...
          <div className="flex flex-col sm:flex-row h-full w-full items-center sm:pl-3 sm:pr-6 sm:pb-2">
            <div className="w-full max-w-96">
              <ChuniAvatar
                className="w-full sm:w-auto sm:h-96"
            ...
            <div className="grid grid-cols-2 w-full px-2 sm:px-0 sm:flex flex-col gap-1.5 sm:ml-3 flex-grow">
              {(
                ...
                (k) =>
                  (k !== 'avatarSkin' ||
                    userboxItems.avatarSkin.length > 1) && (
                    <SelectModalButton
                      key={k}
                      displayMode="grid"
                      modalSize="3xl"
                      ...
                      onSelected={(i) => equipItem(k, i)}
                      items={userboxItems[k]}
                      selectedItem={equipped[k]}
                      renderItem={(i) =>
                        renderItem(i, getImageUrl(`chuni/avatar/${i.iconPath}`))
                      }
                    >
                      Change {k.slice(6)}
                    </SelectModalButton>
          ...
          <div className="flex w-full flex-col sm:flex-row items-center px-2 sm:px-4 sm:pb-4 h-full">
            <div className="flex flex-col">
              <Image
                ...
                height={204}
                priority
                alt={equipped.systemVoice?.name ?? ''}
                src={getImageUrl(
                  `chuni/system-voice-icon/${equipped.systemVoice?.imagePath}`,
                )}
              />
            ...
            <div className="flex flex-col flex-grow w-fullmt-3 sm:-mt-5 sm:w-auto gap-2 w-full">
              <Checkbox
                isSelected={playPreviews}
                onValueChange={setPlayPreviews}
                size="lg"
              ...
              {voicePreview}
              <SelectModalButton
                selectedItem={equipped.systemVoice}
                items={userboxItems.systemVoice}
                displayMode="grid"
                rowSize={150}
                colSize={175}
                gap={6}
                ...
                footer={
                  <>
                    <div className="flex flex-grow gap-2 items-center max-w-full sm:max-w-[min(100%,18rem)]">
                      {voicePreview}
                    </div>
                    <Checkbox
                      isSelected={playPreviews}
                ...
                onSelectionChanged={(i) => {
                  play(
                    getAudioUrl(
                      `chuni/system-voice/${i.cuePath}_${[...selectedLine][0]}`,
                    ),
                  )
                  setSelectingVoice(i)
                }}
                renderItem={(i) =>
                  renderItem(
                    i,
                    getImageUrl(`chuni/system-voice-icon/${i.imagePath}`),
                  )
                }
              >
                Change Voice
        ...
        <div className="flex flex-col w-full col-span-full xl:col-span-6 sm:bg-content1 rounded-lg sm:shadow-inner items-center">
          ...
          <Image
            className="w-52 max-w-full -mt-2"
            width={208}
            height={208}
            priority
            alt={equipped.mapIcon?.name ?? ''}
            src={getImageUrl(`chuni/map-icon/${equipped.mapIcon?.imagePath}`)}
          />
          <span className="text-center mb-2">{equipped.mapIcon?.name}</span>
          <div className="px-2 w-full flex justify-center">
            <SelectModalButton
              onSelected={(i) => i && equipItem('mapIcon', i)}
              selectedItem={equipped.mapIcon}
              displayMode="grid"
              modalSize="full"
              rowSize={210}
              colSize={175}
              items={userboxItems.mapIcon}
              gap={6}
              className="w-full sm:w-auto mb-4"
              modalId="map-icon"
              itemId="id"
              renderItem={(i) =>
                renderItem(i, getImageUrl(`chuni/map-icon/${i.imagePath}`))
              }
            >
              Change Map Icon
            </SelectModalButton>
        ...
        {Object.values(saved).some((x) => !x) && (
          <Button
            className="fixed bottom-3 right-3 hidden sm:flex"
            color="primary"
            radius="full"
            startContent={<SaveIcon className="h-6" />}
            onPress={() => save()}
          >
            Save All
          </Button>
