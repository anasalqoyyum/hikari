...
export const UserSettings = () => {
  ...
  return (
    <section className="w-full rounded-lg sm:bg-content1 sm:shadow-lg">
      ...
      <section className="px-3 pb-4 flex flex-col gap-4">
        ...
        >
          {homepageRoutes.map(({ name, routes }, i) => (
            <SelectSection
              key={i}
              title={name}
              showDivider={i < homepageRoutes.length - 1}
            >
              {routes.map(({ name: subrouteName, url }) => (
                <SelectItem
                  key={url}
                  value={url}
                  textValue={`${name}┃${subrouteName}`}
                >
                  {subrouteName}
                </SelectItem>
              ))}
        ...
        <div className="flex flex-col gap-1">
          ...
          {[...USER_VISIBILITY_NAMES].map(([mask, name]) => (
            <Checkbox
              key={mask}
              isSelected={!!(mask & visibility)}
              isDisabled={
                loading ||
                (mask !== UserVisibility.EVERYONE &&
                  !!(visibility & UserVisibility.EVERYONE))
              }
              onValueChange={(s) => {
