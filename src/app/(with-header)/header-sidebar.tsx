...
export const HeaderSidebar = ({ children }: HeaderSidebarProps) => {
  ...
  const [notificationsTranslate, setNotificationsTranslate] = useState<
    number | null
  >(null)
  ...
  const from = cookies.get('hikari-navigated-from')
  const filter = filterRoute.bind(null, user)
  const routeGroup = ROUTES.find(
    (route) => route.title === from || path?.startsWith(route.url),
  )!
  ...
  useWindowListener('scroll', () => {
    ...
    else if (scrollOffset.current > 100) setNavbarShowing(false)
  })
  ...
  const { ref } = useSwipeable({
    ...
    onSwiped: (e) => {
      ...
      if (menuTranslate && (Math.abs(menuTranslate) > 384 * 0.5 || fastSwipe)) {
        if (isMenuOpen && window.location.hash === '#sidebar') back()
        setMenuOpen(!isMenuOpen)
      } else if (
        notificationsTranslate &&
        (Math.abs(notificationsTranslate) > window.innerWidth * 0.4 ||
          fastSwipe)
      ) {
    ...
    onSwipeStart: (e) => {
      ...
      if (
        (isMenuOpen && e.dir === 'Left') ||
        (allMenusClosed && e.dir === 'Right' && xPercent <= 0.6)
      ) {
        setMenuTranslate(e.deltaX)
        document.body.classList.add('touch-none', 'overflow-hidden')
      } else if (
        (isNotificationsOpen && e.dir === 'Right') ||
        (allMenusClosed && e.dir === 'Left' && xPercent >= 0.4)
      ) {
  ...
  useEffect(() => {
    if (isMenuOpen || isNotificationsOpen)
      document.body.classList.add('touch-none', 'overflow-hidden')
    else document.body.classList.remove('touch-none', 'overflow-hidden')
  }, [isMenuOpen, isNotificationsOpen])

  useEffect(() => {
    if (user) getFriendRequests().then(setFriendRequests)
  }, [pathname, user])
  ...
  const setNotificationsOpen = (open: boolean, updateRef = true) => {
    ...
    if (open) {
      ...
      if (updateRef) menusOpened.current = true
    }
  }
  ...
  const renderHeaderLink = useCallback(
    (route: Subroute) => {
      ...
      if (route.routes?.length)
        return (
          <Dropdown key={route.url}>
            <DropdownTrigger>
              <Button
                className="bg-transparent p-0 m-0 min-h-0 min-w-0 h-auto overflow-visible"
                disableRipple
              >
                <div
                  className={`flex items-center cursor-pointer mr-4 transition text-lg ${linkStyle}`}
                >
                  {route.name}
            ...
            <DropdownMenu>
              {route.routes.filter(filter).map((route) => (
                <DropdownItem
                  key={route.url}
                  className="[&:hover_*]:text-secondary p-0"
                  onPress={() => {
                    cookies.set('hikari-navigated-from', routeGroup.title)
                  }}
                >
                  <Link
                    href={route.url}
                    className={`w-full h-10 pl-2 items-center flex transition text-medium ${
                      path?.startsWith(route.url)
                        ? 'font-semibold text-primary'
                        : ''
                    }`}
                  >
  ...
  const notifications = useMemo(() => {
    ...
    return friendRequests.map((req) => {
      ...
      return (
        <section
          key={req.reqUuid}
          className="flex flex-col h-32 w-full bg-content1 px-4 py-2.5 rounded-lg border-gray-500/25 border mb-2 mr-1"
        >
          <time dateTime={req.createdDate.toISOString()} className="text-xs ">
            {req.createdDate.toLocaleDateString(
              undefined,
          ...
          <div>
            ...
            <Link
              href={`/user/${req.userUuid}`}
              className="font-semibold underline transition hover:text-secondary"
              onClick={() => {
  ...
  const topNavbar = useMemo(() => {
    return (
      <Navbar
        className={`w-lvw fixed transition ${
          !navbarShowing && breakpoint === undefined ? '-translate-y-full' : ''
        }`}
        classNames={{ wrapper: 'max-w-full p-0' }}
        height="5.5rem"
      >
        <div className="flex h-header px-6 items-center flex-shrink-0 w-full z-[48]">
          <Button
            className="text-2xl font-bold cursor-pointer flex items-center m-0 ps-1.5 pe-2 mr-6"
            variant="light"
          ...
          {routeGroup !== MAIN_ROUTES && (
            <div className="mr-4 mt-1 hidden [@media(min-width:1175px)]:flex text-lg">
          ...
          {user ? (
            <>
                ...
                <Popover>
                  <PopoverTrigger>
                    <div className="hidden sm:block">
                      <Badge
                        content={friendRequests.length.toString()}
                        color="danger"
                        shape="circle"
                      >
              ...
              <Dropdown
                isOpen={userDropdownOpen && !isNotificationsOpen}
                onOpenChange={(o) => {
                  if (!o || breakpoint) setUserDropdownOpen(o)
              ...
              >
                <DropdownTrigger>
                  <div className="flex items-center">
                    <Badge
                      ...
                      color="danger"
                      shape="circle"
                      isInvisible={!!breakpoint || !friendRequests.length}
                      size="lg"
                    >
                      <Avatar
                        name={user.username?.[0]?.toUpperCase() ?? undefined}
                        className={`w-12 h-12 ml-2.5 cursor-pointer text-2xl [font-feature-settings:"fwid"]`}
                      />
                    </Badge>
                  </div>
                </DropdownTrigger>
                <DropdownMenu>
                  <DropdownItem showDivider className="p-0">
                    <Link
                      className="text-lg font-semibold block w-full h-full px-2 py-1.5"
                      href={`/user/${user.uuid}`}
                    >
                      {user.username}
                    </Link>
                  ...
                  >
                    <div
                      className="w-full flex pl-2 h-8 items-center"
                      onClick={() =>
                  ...
                  <DropdownItem className="p-0" color="danger" variant="flat">
                    <div
                      className="w-full h-full block px-2 py-1.5 text-danger"
                      onClick={() =>
                        logout({ redirectTo: '/' }).then((r) =>
                          setTimeout(() => (location.href = r.redirect), 10),
                        )
                      }
                    >
                      Logout
  ...
  const leftSidebar = useMemo(() => {
    return (
      <div
        className={`fixed inset-0 w-full h-full z-[49] ${
          isMenuOpen ? '' : 'pointer-events-none'
        }`}
      >
        <div
          className={`transition bg-black z-[49] absolute inset-0 w-full h-full ${
            isMenuOpen ? 'bg-opacity-25' : 'bg-opacity-0 pointer-events-none'
          }`}
          onClick={() => {
            setMenuOpen(false)
            if (window.location.hash === '#sidebar') back()
          }}
        />

        <div
          className={`dark flex flex-col text-white absolute p-6 top-0 h-full max-w-full w-96 bg-gray-950 z-[49] left-0 ${
            menuTranslate ? '' : 'transition-all'
          } ${isMenuOpen ? 'shadow-2xl' : '-translate-x-full'}`}
          style={
            menuTranslate
              ? {
                  transform: `translateX(clamp(-100%, calc(${
                    isMenuOpen
                      ? `${menuTranslate}px`
                      : `${menuTranslate}px - 100%`
                  }), 0px))`,
                }
              : undefined
          }
        >
          <div className="flex">
            <Button
              className="text-2xl mb-6 font-bold cursor-pointer flex items-center ps-1.5 pe-2"
              variant="light"
              onClick={() => {
                setMenuOpen(false)
                if (window.location.hash === '#sidebar') back()
                ...
                <div>
                  {!filter(route) ? (
                    <div
                      className={`select-none text-2xl ${
                        route === routeGroup ? 'font-bold' : 'font-semibold'
                      }`}
                    >
                      {route.name}
                    </div>
                    ...
                    <Link
                      ...
                      onClick={() => {
                        setMenuOpen(false)
                        cookies.remove('hikari-navigated-from')
                      }}
                      className={`block py-1 sm:py-0 text-3xl sm:text-2xl transition hover:text-secondary ${
                        route === routeGroup ? 'font-bold' : 'font-semibold'
                      }`}
                    >
                      {route.name}
                  ...
                  <div className="ml-2 mt-2">
                    {route.routes?.filter(filter)?.map((subroute) => (
                      <div className="mb-1" key={subroute.url}>
                        {subroute.routes?.length ? (
                          <div className={`text-2xl sm:text-xl`}>
                            ...
                            <div className="flex flex-col ml-1.5 pl-3 border-l border-gray-500/25 mt-0.5">
                              {subroute.routes.filter(filter).map((route) => (
                                <Link
                                  href={route.url}
                                  key={route.url}
                                  onClick={() => setMenuOpen(false)}
                                  className={`py-2 sm:py-0 text-xl sm:text-[1.075rem] transition ${
                                    path?.startsWith(route.url)
                                      ? 'font-semibold text-primary'
                                      : 'hover:text-secondary'
                                  }`}
                          ...
                          <Link
                            ...
                            className={`block py-1 sm:py-0 text-2xl sm:text-xl transition ${
                              path?.startsWith(subroute.url)
                                ? 'font-semibold text-primary'
                                : 'hover:text-secondary'
                            }`}
                          >
          ...
          <Button
            ...
            onClick={() =>
              (user ? logout({ redirectTo: '/' }) : login()).then((r) =>
                setTimeout(() => (location.href = r.redirect!), 10),
              )
            }
          >
  ...
  const rightSidebar = useMemo(() => {
    return (
      <div
        className={`fixed inset-0 w-full h-full max-h-full z-[49] ${
          isNotificationsOpen ? '' : 'pointer-events-none'
        }`}
      >
        <div
          className={`flex flex-col dark text-white absolute pt-6 pb-3 top-0 h-full max-w-full w-full max-h-full bg-gray-950 z-[49] ${
            notificationsTranslate ? '' : 'transition-all'
          } left-0 ${
            isNotificationsOpen
            ...
            notificationsTranslate
              ? {
                  transform: `translateX(clamp(0px, ${
                    isNotificationsOpen
                      ? `${notificationsTranslate}px`
                      : `100% + ${notificationsTranslate}px`
                  }, 100%))`,
                }
              : undefined
          }
        >
          <header className="font-semibold text-2xl flex items-center pr-4 pl-2 w-full">
            ...
            <Link
              ...
              onClick={() => setNotificationsOpen(false)}
            >
              <Button className="min-w-0">Profile</Button>
            </Link>
            <Button
              isIconOnly
              className="ml-2"
              ...
              onPress={() => {
                setNotificationsOpen(false)
                if (window.location.hash === '#notifications') back()
              }}
            >
          ...
          <Link
            href="/settings"
            className="font-semibold text-2xl p-3 pl-4 flex items-center"
            onClick={() => setNotificationsOpen(false)}
          >
            Settings
          ...
          <section className="px-2 flex flex-col flex-auto overflow-y-auto mb-3">
            <header className="font-semibold text-2xl px-2 mb-3">
              Notifications
            </header>
            {!friendRequests.length && (
          ...
          <Button
            className="mt-auto mx-3 flex-shrink-0"
            color="danger"
            onPress={() =>
              logout({ redirectTo: '/' }).then((r) =>
                setTimeout(() => (location.href = r.redirect), 10),
