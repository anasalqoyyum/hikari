...
export default async function TeamPage() {
  ...
  const teams = (await getTeams({ user })).filter((t) => t.visible)
  ...
  return (
    <main className="flex flex-col max-w-5xl mx-auto w-full">
      ...
      <section className="w-full px-1 sm:p-0 sm:mt-4 flex flex-col gap-2 mx-auto">
        {!teams.length && (
          <span className="italic text-gray-500 ml-2">No teams found</span>
        )}
        ...
        {teams.map((team) => (
          ...
          >
            ...
            <Link
              href={`/team/${team.uuid}`}
              className="font-semibold transition hover:text-secondary"
            >
              {team.name}
            </Link>
            {!!team.isMember && (
              <Tooltip content="Your team">
                <StarIcon className="h-6 text-amber-400" />
              </Tooltip>
            )}

            <Tooltip
              content={`${team.userCount} User${team.userCount === 1 ? '' : 's'}`}
            >
