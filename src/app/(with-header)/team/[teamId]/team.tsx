...
type TeamDetailProps = {
  team: Team
  users: TeamUser[]
  links: DB['hikari_team_join_keys'][]
}
...
export const TeamDetail = ({
  ...
  const isOwner =
    ...
    (user && user.uuid === team.ownerUuid)

  return (
    <main className="w-full flex flex-col mt-2">
      <JoinLinksModal
        links={links}
        prefix={`/team/${team.uuid}/join/`}
      ...
      <header className="font-bold text-5xl self-center flex gap-3 items-center">
        ...
        {editing ? (
          <Input
            ...
            placeholder="Name"
            value={team.name ?? ''}
            onChange={(ev) => setTeam((t) => ({ ...t, name: ev.target.value }))}
            classNames={{
      ...
      <section
        className={`flex ${
          editing ? 'flex-col gap-x-2' : 'gap-x-10'
        } sm:flex-row w-full justify-center mt-5 gap-y-3 text-xl items-center`}
      >
        {editing ? (
          <>
            <Select
              isRequired
              label="Join Privacy"
              selectedKeys={new Set([team.joinPrivacy.toString()])}
              className="w-full px-2 sm:w-60 sm:ml-14"
              isDisabled={loading}
              size="sm"
              onSelectionChange={(s) =>
                typeof s !== 'string' &&
                s.size &&
                setTeam((t) => ({ ...t, joinPrivacy: +[...s][0] }))
              }
            >
              <SelectItem key={JoinPrivacy.INVITE_ONLY.toString()}>
                Invite Only
              </SelectItem>
              <SelectItem key={JoinPrivacy.PUBLIC.toString()}>
                Public
              </SelectItem>
            </Select>
            ...
            <div className="h-full flex gap-2 min-h-12 ml-auto mr-2 sm:m-0">
              ...
              <Button className="h-full" color="primary" onPress={save}>
                Save
              </Button>
            </div>
          </>
          ...
          <>
            ...
            <div>
              ...
              {team.joinPrivacy === JoinPrivacy.PUBLIC
                ? 'Public'
                : 'Invite Only'}
            </div>

            {isOwner && (
              <Tooltip content="Edit team settings">
                <Button
                  isIconOnly
                  variant="light"
                  radius="full"
      ...
      <section className="max-w-screen-4xl w-full mx-auto">
        <header className="py-4 pl-4 sm:pl-0 flex items-center text-2xl font-semibold">
          ...
          {(team.joinPrivacy === JoinPrivacy.PUBLIC || isOwner) &&
            !isMember &&
            !user?.team && (
              <Tooltip content="Join this team">
                <Button
                  className="mr-2"
                  isIconOnly
                  size="lg"
                  onPress={() =>
                    joinPublicTeam(team.uuid).then((res) => {
                      if (res?.error) return setError(res.message)
                      location.reload()
                    })
                  }
          ...
          {!isOwner && isMember && (
            <Tooltip
              content={<span className="text-danger">Leave this team</span>}
            >
              <Button
                className="mr-2"
                isIconOnly
                ...
                onPress={() => {
                  confirm('Would you like to leave this team?', () => {
                    removeUserFromTeam(team.uuid).then((res) => {
                      if (res?.error) return setError(res.message)
                      location.reload()
                    })
                  })
        ...
        <div className="flex flex-wrap gap-3 px-2 sm:px-0">
          {users.map((teamUser, index) => (
            ...
            >
              ...
              {isOwner && teamUser.uuid !== user?.uuid && (
                <Tooltip
                  ...
                  <Button
                    ...
                    onPress={() => {
                      confirm('Are you sure you want to kick this user?', () =>
                        removeUserFromTeam(team.uuid, teamUser.id!).then(() =>
                          setUsers((u) =>
                            u.filter((u) => u.uuid !== teamUser.uuid),
                          ),
                        ),
                      )
                    }}
          ...
          <section className="max-w-screen-4xl w-full mx-auto">
            ...
            <Button
              ...
              onPress={() => {
                confirm(
                  <span>
                    Do you want to delete this team? This will remove all team
                    data such as team points.
