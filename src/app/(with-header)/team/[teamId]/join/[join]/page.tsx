...
export default async function Join({
  ...
  params: { teamId: string; join: string }
}) {
  const user = await requireUser()

  if (!params.join) return <InvalidLink />

  if (user.team === params.teamId) return redirect(`/team/${params.teamId}`)

  if (user.team)
    return (
      <main className="flex flex-col w-full m-auto items-center gap-4 pb-10 text-center">
  ...
  const joinLink = await db
    ...
    .innerJoin('hikari_team_join_keys as key', 'key.teamId', 'team.uuid')
  ...
  await db.transaction().execute(async (trx) => {
    ...
    if (joinLink.remainingUses !== null && joinLink.remainingUses <= 1)
      ...
      await trx
        .updateTable('hikari_team_join_keys')
        .where('id', '=', joinLink.id)
        .set({
