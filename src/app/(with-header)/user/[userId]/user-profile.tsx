...
export type UserProfile<V extends boolean> = Pick<
  UserPayload,
  'username' | 'id' | 'uuid' | 'permissions'
> &
  Pick<DBUserPayload, 'created_date' | 'last_login_date'> & { visible: V }
type UserFriend =
  | Pick<DB['hikari_user_friends'], 'chuniRival'>
  | null
  | undefined
...
export const UserProfile = <T extends boolean>(props: UserProfileProps<T>) => {
    ...
    <>
      <header className="flex flex-wrap w-full text-4xl font-bold mt-4 px-4 sm:mt-12 max-w-4xl mx-auto items-center gap-3">
        <div className="flex items-center mx-auto sm:mx-0 flex-wrap gap-y-2">
          {!!props.friend?.chuniRival && (
            <Tooltip content="Chunithm Rival">
              <div>
                <ChuniPenguinIcon className="h-8 mr-3 md:-ml-8 ml-2" />
              </div>
            </Tooltip>
          )}
          ...
          {[...USER_PERMISSION_NAMES]
            .filter(
              ([permission]) => props.user.permissions! & (1 << permission),
            )
            .map(([permission]) => (
              <PermissionIcon
                key={permission}
        ...
        <div className="ml-auto flex gap-2">
          {hasPermission(viewingUser?.permissions, UserPermissions.USERMOD) && (
            <Link href={`/admin/users#${props.user.uuid}`}>
          ...
          {viewingUser?.id !== props.user.id && (
            <>
              {props.friend ? (
                <Tooltip
                  content={<span className="text-danger">Unfriend</span>}
                >
                  <Button
                    isIconOnly
                    size="lg"
                    ...
                    onPress={() =>
                      confirm(
                        `Do you want to unfriend ${props.user.username}?`,
                        () => {
                          unfriend(props.user.id).then(() => location.reload())
                        },
                      )
                    }
                  >
      ...
      <div className="max-w-4xl mx-auto w-full flex mt-4 px-2">
        ...
        <span className="ml-auto text-right">
          Last Seen{' '}
          {props.user.last_login_date?.toLocaleDateString(undefined, FORMAT)}
        </span>
      </div>
  ...
  if (!props.user.visible)
    return (
      <main className="flex flex-col">
        {header}
        <div className="italic text-gray-500 mx-auto max-w-7xl w-full text-xl font-light pl-6">
  ...
  return (
    <main className="flex flex-col">
      {header}
      {chuniProfile && (
        <section className="w-full flex flex-col max-w-7xl mx-auto">
          <header className="mb-8 font-semibold text-2xl px-4">
            Chunithm Profile
          </header>
