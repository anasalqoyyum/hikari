...
export default async function UserProfilePage({
  ...
  const user = structuredClone(
    await withUsersVisibleTo(viewingUser)
  ...
  const [friend, pendingFriend] = await Promise.all([
    db
      .selectFrom('hikari_user_friends')
      .where('user1', '=', user.id)
  ...
  if (!user.visible)
    return (
      <UserProfile
        ...
        user={user as UserProfile<false>}
      />
    )
  ...
  return (
    <UserProfile
      friend={friend}
      pendingFriend={!!pendingFriend}
      user={user as UserProfile<true>}
      chuniProfile={chuniProfile}
    />
  )
}
