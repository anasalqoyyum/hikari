...
import { HikariStatus } from '@/components/hikari-status'
...
type Item<D> = {
  ...
  render: IsEqual<D, undefined> extends true
    ? () => ReactNode
    : (data: D) => ReactNode
...
type ItemData = {
  ...
  'hikari-status': ServerStatus
}

const NO_CHUNI_PROFILE = (
...
const ITEMS: { [K in keyof ItemData]: Item<ItemData[K]> } = {
  'chuni-nameplate': {
    aspect: [576, 228],
    render: (d) =>
      d ? <ChuniNameplate profile={d} className="h-full" /> : NO_CHUNI_PROFILE,
    available: (u) => !!u?.chuni,
  ...
  'hikari-status': {
    render: (s) => <HikariStatus status={s} className="w-full h-full" />,
    minH: 225,
    name: 'Hikari Status',
  },
}
...
const centerX = ({ w, col }: { w: number; col: number }) =>
  Math.floor((col - w) / 2)
...
const applyLayoutProps = (layouts: Layouts): Layouts =>
  Object.fromEntries(
    Object.entries(layouts).map(([k, layout]) => [
      k,
      layout.map((l) => {
        ...
        [...layout]
          ...
          .map(({ x, y, w, h, i }) => {
            ...
            return {
              ...
              w,
              i,
              h: ITEMS[i as keyof ItemData].aspect ? 1 : h,
            }
          }),
      ]
    }),
...
export const Dashboard = ({
  ...
  const [layouts, setLayouts] = useState<Layouts>(
    applyLayoutProps(initialDashboard ?? getDefaultLayout(user)),
  )
  ...
  const reflowItemAspect = useCallback(
    (keepWidth: boolean, ...items: GridLayout.Layout[]) => {
      ...
      items.forEach((layoutItem) => {
        ...
        if (keepWidth) {
          layoutItem.h = Math.max(
            Math.round(
              (layoutItem.w * colWidth + margin[0]) / aspect + margin[1],
            ),
            1,
          )
          return
        ...
        layoutItem.w = Math.min(
          Math.max(
            Math.round((layoutItem.h * aspect + margin[0]) / colWidth),
            1,
          ),
          col,
        )
  ...
  useEffect(() => {
    setLayouts((l) => ({
      ...l,
      [col]: reflowItemAspect(true, ...l[col].map((i) => ({ ...i }))),
    }))
  ...
  const children = useMemo(() => {
    return layouts[col.toString()].map((layout) => {
      ...
      else if (layout.i === 'hikari-status')
        rendered = ITEMS['hikari-status'].render(serverStatus)
      ...
      return (
        <div
          ...
          className={`${marginClass} ${
            editing ? 'bg-gray-500/25 select-none' : ''
          } ${resizingWindow ? '!transition-none' : ''}`}
        >
          <div
            className={`w-full max-h-full ${item.aspect ? '' : 'h-full'}`}
            style={
              item.aspect
  ...
  const headerButtons = useMemo(() => {
    ...
    const usedItems = new Set(
      Object.values(layouts).flatMap((l) =>
        l.map((item) => item.i),
      ) as (keyof ItemData)[],
    )
    ...
    const removeItem = (item: keyof ItemData) => {
      setLayouts((layouts) =>
        Object.fromEntries(
          Object.entries(layouts).map(([key, items]) => [
            key,
            items.filter((i) => i.i !== item),
          ]),
        ),
      )
    }

    const availablePreviews = [...OVERRIDE_COLS].filter(
      ([col]) => col < trueCol,
    )
    const previewItems = [
      <DropdownItem
    ...
    return (
      <>
        {(!!availablePreviews.length || overrideCol !== null) && (
          <Dropdown>
            ...
            {(Object.entries(ITEMS) as Entries<typeof ITEMS>)
              ...
              .map(([key, item]) => (
                <DropdownItem
                  key={key}
                  onPress={() =>
                    usedItems.has(key) ? removeItem(key) : addItem(key)
                  }
                >
                  {item.name}
                </DropdownItem>
        ...
        <Tooltip content="Discard Changes">
          <Button
            ...
            onPress={() => {
              ...
              setLayouts(
                Object.fromEntries(
                  Object.entries(layoutRestore.current).map(([k, v]) => [
                    k,
                    reflowItemAspect(true, ...v.map((i) => ({ ...i }))),
                  ]),
                ),
              )
            }}
  ...
  const handle = (
    <div className="h-full py-2 px-3 w-px cursor-pointer relative">
  ...
  const handlePreviewResize = (_: any, data: ResizeCallbackData) => {
    ...
    const [col] = [...OVERRIDE_COLS].findLast(
      ([col, { breakpoint }]) => newWidth >= breakpoint,
    )!
    setOverrideCol(col)
  ...
  return (
    <main className="flex w-full h-full">
      {showResize && (
        ...
        >
          <div
            className={`flex-grow flex justify-end bg-gray-400/25 h-full w-2 sm:-ml-5 ${
              overrideCol === COLS.xs ? '' : 'sm:mr-5'
            }`}
          />
        </Resizable>
      )}
      <section
        className={`pb-4 overflow-hidden h-full w-full flex-shrink-0 ${
          mounted ? '' : 'opacity-0'
        }`}
        ref={containerRef}
      ...
      {showResize && (
        ...
        >
          <div
            className={`flex-grow bg-gray-400/25 h-full sm:-mr-5 ${
              overrideCol === COLS.xs ? '' : 'sm:ml-5'
            }`}
          />
        </Resizable>
      )}
