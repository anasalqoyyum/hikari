    ...
    <section>
      <section className="px-3 sm:bg-content1 sm:rounded-lg py-2.5 flex items-center">
        <Link href={`/user/${friend.uuid}`} className="flex items-center">
          ...
          />
          <span className="font-semibold transition hover:text-secondary">
            {friend.username}
          </span>
        </Link>
        {!!friend.chuniRival && (
        ...
        <Dropdown>
          ...
          <DropdownMenu>
            <DropdownItem
              onPress={() => {
                if (friend.chuniRival)
                  removeFriendAsRival(friend.id).then(() =>
                    setFriend({ ...friend, chuniRival: 0 }),
                  )
                else
            ...
            <DropdownItem
              ...
              onPress={() =>
                confirm(
                  'Are you sure you want to unfriend this user? This will also remove them as a rival.',
                  () => {
                    unfriend(friend.id).then(onUnfriend)
                  },
                )
              }
            >
