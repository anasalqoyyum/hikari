...
import {
  ...
  deleteArcadeLink,
  joinPublicArcade,
  removeUserFromArcade,
  setUserArcadePermissions,
  updateArcade,
} from '@/actions/arcade'
...
import {
  PermissionEditModal,
  PermissionEditModalUser,
} from '@/components/permission-edit-modal'
...
const getArcadeLabel = (k: string) => {
  ...
  return `${k[0].toUpperCase()}${k.slice(1).replace(/_id$/gi, ' ID')}`
}

const getArcadeValue = <
  T extends {
...
>(
  ...
  if (k === 'region_id')
    return (
      ALLNET_JAPAN_REGION.get(+(arcade.region_id as any)) ?? arcade.region_id
    )
...
export const ArcadeDetail = ({
  ...
  const [arcade, setArcade] = useState({
    ...initialArcade,
    region_id: initialArcade?.region_id?.toString(),
  ...
  const [editingUser, setEditingUser] =
    useState<PermissionEditModalUser | null>(null)
  const router = useRouter()

  const save = () => {
    const arcadeUpdateVals = Object.fromEntries(
      Object.entries(arcade).filter(([k]) => ARCADE_UPDATE_KEYS.includes(k)),
    )

    const update: Partial<ArcadeUpdate> = {
      ...arcadeUpdateVals,
      country_id:
        arcade.country_id === '' || arcade.country_id === undefined
          ? null
          : +arcade.country_id,
      region_id: null,
    }
    ...
    ;(Object.entries(update) as Entries<typeof update>).forEach(([k, v]) => {
      if (typeof v === 'string') (update as any)[k] = v = v.trim()
    ...
    if (k === 'country')
      return (
        <Select
          ...
          onSelectionChange={(s) =>
            typeof s !== 'string' &&
            setArcade((a) => ({ ...a, country: [...s][0]?.toString() ?? null }))
          }
        >
    ...
    if (k === 'region_id')
      return (
        <Autocomplete
          ...
          onSelectionChange={(s) =>
            setArcade((a) => ({ ...a, region_id: s?.toString() }))
          }
        >
          {
            [
              ...[...ALLNET_JAPAN_REGION].map(([regionId, name]) => (
                <AutocompleteItem
                  key={regionId.toString()}
                  textValue={`${name} (${regionId})`}
                >
                  {name} ({regionId})
                </AutocompleteItem>
              )),
  ...
  return (
    <main className="w-full flex flex-col mt-2">
      <JoinLinksModal
        links={links}
        prefix={`/arcade/${arcade.uuid}/join/`}
      ...
      <PermissionEditModal
        ...
        displayUpTo={
          hasPermission(user?.permissions, UserPermissions.ACMOD)
            ? ArcadePermissions.OWNER
            : ArcadePermissions.REGISTRAR
        }
        onEdit={(user, permissions) => {
          setUsers((u) =>
            u.map((u) => (u.id === user ? { ...u, permissions } : u)),
          )
          setUserArcadePermissions({
            arcadeUser: user,
            permissions,
      ...
      <header className="font-bold text-5xl self-center flex gap-3 items-center">
        <VisibilityDropdown
          visibility={arcade.visibility}
          editing={editing}
          loading={loading}
          onVisibilityChange={(v) =>
            setArcade((a) => ({ ...a, visibility: v }))
          }
        />
        {editing ? (
          <Input
            ...
            value={arcade.name ?? ''}
            onChange={(ev) =>
              setArcade((a) => ({ ...a, name: ev.target.value }))
            }
            classNames={{
      ...
      {editing ? (
        <section className="grid px-2 sm:px-0 grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-5 5xl:grid-cols-9 gap-2 flex-wrap mt-3">
          {ARCADE_KEYS.map(renderEdit)}
          <Select
            isRequired
            label="Join Privacy"
            selectedKeys={new Set([arcade.joinPrivacy.toString()])}
            isDisabled={loading}
            onSelectionChange={(s) =>
              typeof s !== 'string' &&
              s.size &&
              setArcade((a) => ({ ...a, joinPrivacy: +[...s][0] }))
            }
          >
            <SelectItem key={JoinPrivacy.INVITE_ONLY.toString()}>
              Invite Only
            </SelectItem>
            <SelectItem key={JoinPrivacy.PUBLIC.toString()}>Public</SelectItem>
          </Select>

          <div className="col-span-2 sm:col-span-3 xl:col-span-1 5xl:col-span-1 5xl:col-start-5 grid grid-cols-2 gap-2 h-full min-h-10">
            <Button
              variant="light"
              color="danger"
              className="h-full"
        ...
        <section className="self-center mt-5 gap-x-8 gap-y-4 items-center justify-center text-xl flex flex-wrap">
          {ARCADE_KEYS.map(
            (k) =>
              arcade[k] && (
                <span key={k}>
                  <span className="font-semibold">{getArcadeLabel(k)}:</span>{' '}
                  {getArcadeValue(arcade, k)}
                </span>
              ),
          )}
          <span>
            <span className="font-semibold">Join Privacy: </span>
            {arcade.joinPrivacy === JoinPrivacy.INVITE_ONLY
              ? 'Invite only'
              : 'Public'}
          </span>
          {hasArcadePermission(
      ...
      <section className="max-w-screen-4xl w-full mx-auto">
        <header className="py-4 pl-4 sm:pl-0 flex items-center text-2xl font-semibold">
          ...
          {!creatingNewCab && (
            <Button
              ...
              isIconOnly
              size="lg"
              onPress={() => setCreatingNewCab(true)}
            >
        ...
        <section className="px-2 sm:px-0">
          {creatingNewCab && (
            <Cab
              permissions={arcade.permissions}
          ...
          {cabs.length || creatingNewCab ? (
            cabs.map((cab) => (
              <Cab
                ...
                permissions={arcade.permissions}
                onEdit={(newCab) =>
                  setCabs((c) => c.map((c) => (c.id === cab.id ? newCab : c)))
                }
                onDelete={() =>
                  setCabs((c) => c.filter((c) => c.id !== cab.id))
                }
              />
            ))
          ) : (
            <span className="italic text-gray-500">
              This arcade has no machines
            </span>
          )}
        </section>
      </section>
      ...
      <section className="max-w-screen-4xl w-full mx-auto">
        <header className="py-4 pl-4 sm:pl-0 flex items-center text-2xl font-semibold">
          ...
          {(arcade.joinPrivacy === JoinPrivacy.PUBLIC ||
            hasPermission(user?.permissions, UserPermissions.ACMOD)) &&
            !arcade.permissions && (
              <Tooltip content="Join this arcade">
                <Button
                  className="mr-2"
                  isIconOnly
                  size="lg"
                  onPress={() =>
                    joinPublicArcade(arcade.id).then(() => location.reload())
                  }
                >
          ...
          {(hasPermission(arcade.permissions, ArcadePermissions.OWNER) ||
            ...
            <Tooltip content="Manage invite links">
              <Button
                className="mr-2"
                isIconOnly
                size="lg"
          ...
          {!!arcade.permissions &&
            !hasPermission(arcade.permissions, ArcadePermissions.OWNER) && (
              <Tooltip
                content={<span className="text-danger">Leave this arcade</span>}
              >
                <Button
                  className="mr-2"
                  isIconOnly
                  ...
                  onPress={() => {
                    confirm('Would you like to leave this arcade?', () => {
                      removeUserFromArcade(arcade.id).then(() =>
                        location.reload(),
                      )
                    })
                  }}
        ...
        {!users.length && (
          <span className="italic text-gray-500">This arcade has no users</span>
        )}
          ...
          {users.map((arcadeUser, index) => (
            ...
            >
              {'username' in arcadeUser ? (
                <Link
                  className="font-semibold underline transition hover:text-secondary mr-auto"
                  href={`/user/${arcadeUser.uuid}`}
                >
                  {arcadeUser.username}
                </Link>
              ) : (
                <span className="text-gray-500 italic mr-auto">
                  Anonymous User
                </span>
              )}
              ...
              {(hasPermission(arcade.permissions, ArcadePermissions.OWNER) ||
                ...
                <Tooltip content="Edit user permissions">
                  <Button
                    isIconOnly
                    size="sm"
                    onPress={() => setEditingUser(arcadeUser)}
                  >
              ...
              {(hasPermission(arcade.permissions, ArcadePermissions.OWNER) ||
                ...
                arcadeUser.id !== user?.id && (
                  <Tooltip
                    content={<span className="text-danger">Kick user</span>}
                  >
                    <Button
                      isIconOnly
                      color="danger"
                      ...
                      onPress={() => {
                        confirm(
                          ...
                          () =>
                            removeUserFromArcade(arcade.id, arcadeUser.id).then(
                              () =>
                                setUsers((u) =>
                                  u.filter((u) => u.id !== arcadeUser.id),
                                ),
                            ),
                        )
                      }}
      ...
      {hasArcadePermission(
        ...
        <section className="max-w-screen-4xl w-full mx-auto">
          ...
          <Button
            ...
            onPress={() => {
              confirm(
                'Do you want to delete this arcade? This action cannot be undone.',
                () => {
                  deleteArcade(arcade.id).then(() => {
                    router.push('/arcade')
                    router.refresh()
                  })
                },
