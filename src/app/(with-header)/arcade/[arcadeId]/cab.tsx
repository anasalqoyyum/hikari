...
export const Cab = ({
  ...
  const [cab, setCab] = useState(
    initialCab
      ? {
          ...initialCab,
          serial: initialCab?.serial
            ? formatSerial(initialCab.serial)
            : initialCab.serial,
          board: initialCab?.board
            ? formatSerial(initialCab.board)
            : initialCab.board,
        }
      : ({} as ArcadeCab),
  )
  ...
  const renderEdit = (k: keyof ArcadeCab) => {
    if (k === 'game')
      return (
        <Autocomplete
          ...
          onSelectionChange={(s) =>
            setCab((c) => ({ ...c, game: s?.toString() }))
          }
        >
          {[...GAME_IDS].map(([id, name]) => (
            <AutocompleteItem key={id} textValue={`${id} (${name})`}>
              {id} ({name})
            </AutocompleteItem>
          ))}
        </Autocomplete>
    ...
    if (k === 'country')
      return (
        <Select
          ...
          onSelectionChange={(s) =>
            typeof s !== 'string' &&
            setCab((c) => ({ ...c, country: [...s][0]?.toString() ?? null }))
          }
        >
    ...
    if (k === 'ota_enable' || k === 'is_cab')
      return (
        <Checkbox
          className="text-nowrap mr-0.5 row-start-4 my-1"
          size="lg"
          isSelected={!!cab[k]}
          isDisabled={loading}
    ...
    const setFormatSerial = () =>
      setCab((c) => (c.serial ? { ...c, serial: formatSerial(c.serial) } : c))

    if (k === 'serial')
      return (
        <div key={k} className="w-full h-full flex rounded-lg overflow-hidden">
          <Input
            isRequired
            size="sm"
            isDisabled={loading}
          ...
          <Tooltip content="Generate random keychip">
            <Button
              ...
              onPress={() =>
                setCab((c) => ({
                  ...c,
                  serial: formatSerial(generateRandomKeychip()),
                }))
              }
            >
    ...
    return (
      <Input
        key={k}
        value={cab[k]?.toString() ?? ''}
        size="sm"
        isDisabled={loading}
        onValueChange={(val) =>
          setCab((c) => ({
            ...c,
            [k]: k === 'board' ? val.toUpperCase() : val,
          }))
        }
  ...
  if (creatingNew || editing)
    return (
      <section className="p-4 rounded-lg bg-content1 flex flex-col gap-2 mb-2 shadow">
        ...
        <Textarea
          ...
          isDisabled={loading}
          value={cab.memo ?? ''}
          onValueChange={(memo) => setCab((c) => ({ ...c, memo }))}
        />
  ...
  return (
    <section className="p-4 rounded-lg shadow bg-content1 flex flex-wrap text-xs sm:text-sm md:text-medium gap-x-4 gap-y-2 items-center mb-2">
      <header className="text-lg font-semibold">
        {cab.game ? (
          <>
            {GAME_IDS.has(cab.game)
              ? `${GAME_IDS.get(cab.game)} (${cab.game})`
              : cab.game}
          </>
        ) : (
          'Unknown Game'
      ...
      {cab.serial && (
        <span>
          <span className="font-semibold">Keychip: </span>
          {formatSerial(cab.serial)}
        </span>
      )}
      ...
      {cab.country && (
        <span>
          ...
          {COUNTRY_CODES.get(cab.country) ?? cab.country}
        </span>
      )}
      ...
      <span>
        <span className="font-semibold">OTA Enabled: </span>
        {cab.ota_enable ? 'Yes' : 'No'}
      </span>

      <span>
        <span className="font-semibold">Is Cab: </span>
        {cab.is_cab ? 'Yes' : 'No'}
      </span>
      ...
      {hasArcadePermission(
        ...
        <div className="ml-auto flex gap-2">
          <Tooltip
            content={<span className="text-danger">Delete this machine</span>}
          >
            <Button
              isIconOnly
              color="danger"
      ...
      {cab.memo && (
        <summary className="block w-full">
