...
export default async function Join({
  ...
  params: { arcadeId: string; join: string }
}) {
  const user = await requireUser()

  if (!params.join) return <InvalidLink />

  const joinLink = await db
    .selectFrom('hikari_arcade_ext as ext')
    .innerJoin('hikari_arcade_join_keys as key', 'key.arcadeId', 'ext.arcadeId')
    .where('ext.uuid', '=', params.arcadeId)
    .where('key.id', '=', params.join)
    .select(['key.arcadeId', 'key.remainingUses', 'key.totalUses', 'key.id'])
    .executeTakeFirst()

  if (!joinLink) return <InvalidLink />

  const res = await db
    .selectFrom('arcade_owner')
    .where('arcade', '=', joinLink.arcadeId)
    .where('user', '=', user.id)
    .select('user')
    .executeTakeFirst()

  if (res) return redirect(`/arcade/${params.arcadeId}`)
  ...
  await db.transaction().execute(async (trx) => {
    ...
    if (joinLink.remainingUses !== null && joinLink.remainingUses <= 1)
      ...
      await trx
        .updateTable('hikari_arcade_join_keys')
        .where('id', '=', joinLink.id)
        .set({
