...
const getLocation = (arcade: Arcade) => {
  let out = [arcade.city, arcade.state, arcade.country]
    .filter((x) => x)
    .join(', ')
  ...
  return (
    <main className="flex flex-col max-w-5xl mx-auto w-full">
      ...
      <section className="w-full px-1 sm:p-0 sm:mt-4 flex flex-col gap-2 mx-auto">
        {!arcades.length && (
          <span className="text-gray-500 italic ml-2">No arcades found</span>
        )}
        ...
        {arcades.map((arcade) => (
          ...
          >
            ...
            <div className="ml-auto flex flex-wrap items-center">
              {!!arcade.machineCount && (
                <span className="mr-3">
                  {arcade.machineCount!.toString()} Machine
                  {Number(arcade.machineCount) > 1 ? 's' : ''}
                </span>
              )}
              <Tooltip
                content={`${arcade.userCount} User${arcade.userCount === 1 ? '' : 's'}`}
              >
              ...
              {arcade.ownerUuid ? (
                <Link
                  href={`/user/${arcade.ownerUuid}`}
                  className="font-semibold hover:text-secondary transition"
                >
                  {arcade.ownerUsername}
                </Link>
              ) : (
                <span className="text-gray-500 italic">anonymous user</span>
              )}
            </div>
          </article>
        ))}
