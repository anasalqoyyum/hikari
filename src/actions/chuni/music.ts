...
export const getMusic = async (musicId?: number) => {
  ...
  return await db
    .selectFrom('chuni_static_music as music')
    ...
    .leftJoin('chuni_item_favorite as favorite', (join) =>
      join
        .onRef('music.songId', '=', 'favorite.favId')
...
const getMusicById = async (user: UserPayload, musicId: number) => {
  if (isNaN(musicId)) return { error: true, message: 'Invalid music ID.' }
  ...
  if (!music) return { error: true, message: `Unknown music ID ${musicId}.` }
