...
export type GetPlaylogOptions = {
  ...
  ascending?: boolean
  search?: string
} & ({} | { musicId: number } | { musicId: number; chartId: number }) &
  ...
  const builder = db
        ...
        .select(
          ({ ref }) =>
            [
              ...
              'playlog.judgeGuilty',
              'playlog.judgeAttack',
              'playlog.judgeCritical',
              'playlog.judgeHeaven',
              'playlog.playerRating',
              'playlog.judgeJustice',
              'playlog.isNewRecord',
              'playlog.isFullCombo',
              'playlog.fullChainKind',
              ...
              sql<number>`(playlog.playerRating - (LEAD(playlog.playerRating) OVER (ORDER BY id DESC)))`.as(
                'playerRatingChange',
              ),
            ] as const,
        )
    ...
    .where(({ and, eb, or, fn }) =>
      and([
        ...
        ...(opts.difficulty?.size
          ? [
              eb(
                'p.chartId',
                'in',
                [...opts.difficulty].map((x) => +x),
              ),
            ]
          : []),
        ...
        ...(opts.lamp?.has('aj') && opts.lamp?.has('fc')
          ? [or([eb('p.isAllJustice', '=', 1), eb('p.isFullCombo', '=', 1)])] // all justice and full combo selected, return either
          : []),
        ...(opts.lamp?.has('aj') && !opts.lamp?.has('fc')
          ? [eb('p.isAllJustice', '=', 1)]
          : []), // return only all justice
        ...(!opts.lamp?.has('aj') && opts.lamp?.has('fc')
          ? [eb('p.isAllJustice', '=', 0), eb('p.isFullCombo', '=', 1)]
          : []), // return only full combo
        ...(opts.worldsEndTag?.size
          ? [
              or([
    ...
    .orderBy(
      SORT_KEYS_SET.has(opts.sort!)
        ? `${SORT_KEYS[opts.sort as keyof typeof SORT_KEYS]} ${
            opts.ascending ? 'asc' : 'desc'
          }`
        : 'p.id desc',
    )
