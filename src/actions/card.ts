...
export const getCards = async (user: number) => {
  return db
    .selectFrom('aime_card')
    .where('user', '=', user)
    .selectAll()
    .execute()
}

export const banUnbanCard = async (opts: {
  cardId: number
  userId: number
  isBan: boolean
}) => {
  ...
  await db
    .updateTable('aime_card')
    .set({ is_banned: +opts.isBan })
    .where(({ and, eb }) =>
      and([eb('id', '=', opts.cardId), eb('user', '=', opts.userId)]),
    )
    .executeTakeFirst()
...
export const lockUnlockCard = async (opts: {
  cardId: number
  userId: number
  isLock: boolean
}) => {
...
export const adminAddCardToUser = async (
  user: number,
  code: string,
): Promise<ActionResult<{ data: AdminUser[] }>> => {
  await requireUser({ permission: UserPermissions.USERMOD })
...
export const userAddCard = async (
  code: string,
): Promise<ActionResult<{ card: DB['aime_card'] }>> => {
  const user = await requireUser()
  ...
  if (!hasPermission(user.permissions, UserPermissions.USERMOD)) {
    ...
    if (!getGlobalConfig('allow_user_add_card'))
      return {
        error: true,
        message: 'You do not have permissions to add a card',
      }
    ...
    if (cards.length >= (getGlobalConfig('user_max_card') ?? Infinity))
      return {
        error: true,
        message:
          'You cannot add a card because you have reached your max card count',
      }
  }
