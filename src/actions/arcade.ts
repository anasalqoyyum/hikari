...
import { HikariArcadeExt as DBArcadeExt, Arcade as DBArcade } from '@/types/db'
import { Arcade, countryValidator, getArcadePermissions } from '@/data/arcade'
import { db } from '@/db'
import { requireUser } from '@/actions/auth'
import {
  hasPermission,
  requireArcadePermission,
  requirePermission,
...
export type ArcadeUpdate = Partial<
  Pick<
    Arcade,
    | 'visibility'
    | 'joinPrivacy'
...
ARCADE_VALIDATORS.set('joinPrivacy', (val) => {
  if (!PRIVACY_VALUES.has(val!)) throw new Error('Invalid join privacy value')
})
;(['name', 'nickname', 'state', 'city'] as const).forEach((v) =>
  ARCADE_VALIDATORS.set(v, () => {}),
)
;(['country_id', 'region_id'] as const).forEach((v) =>
  ARCADE_VALIDATORS.set(v, (val) => {
    const name = v.split('_')[0]
    if (typeof val !== 'number' || !Number.isInteger(val))
      throw new Error(
        `${name[0].toUpperCase()}${name.slice(1)} ID must be a number`,
      )
  }),
)
...
ARCADE_VALIDATORS.set('ip', (val) => {
  if (!IP_REGEX.test(val ?? '')) throw new Error('Invalid IP format')
})
...
export const updateArcade = async (id: number, update: ArcadeUpdate) => {
  ...
  for (let [key, val] of Object.entries(update) as Entries<ArcadeUpdate>) {
    ...
    try {
      ...
      if (val !== null)
        val =
          (await ARCADE_VALIDATORS.get(key as keyof ArcadeUpdate)!(val)) ?? val
    } catch (e: any) {
    ...
    else arcadeUpdate[key] = val as any
  }
...
export const createArcadeLink = async (
  arcade: number,
  ...
  await db
    .insertInto('hikari_arcade_join_keys')
    .values({
      id,
      arcadeId: arcade,
      remainingUses,
...
export const deleteArcadeLink = async (arcade: number, link: string) => {
  ...
  await db
    .deleteFrom('hikari_arcade_join_keys')
  ...
  await db.transaction().execute(async (trx) => {
    ...
    await trx
      ...
      .values({
        ...
        permissions:
          (1 << ArcadePermissions.OWNER) | (1 << ArcadePermissions.VIEW),
      })
      .executeTakeFirst()
  })
...
export const deleteArcade = async (id: number) => {
  ...
  await db.deleteFrom('arcade').where('id', '=', id).executeTakeFirst()

  revalidatePath('/arcade', 'page')
  redirect('/arcade')
}

export const setUserArcadePermissions = async ({
  arcadeUser,
  arcade,
  permissions,
}: {
