...
const booleanValidator = (val: number | null | undefined) => {
  val = +(val as any)
  if (val !== 0 && val !== 1) throw new Error('Invalid boolean value')
  return val
}
...
CAB_VALIDATORS.set('board', async (val, cab) => {
  ...
  if (existing) throw new Error('That board is already in use')

  return val
})

const getMachineBySerial = async (
  serial: string,
  excludeCab: number | null = null,
) => {
  return db
    .selectFrom('machine')
    .where(({ eb, and }) =>
      and([
        ...
        ...(excludeCab === null ? [] : [eb('id', '!=', excludeCab)]),
      ]),
    )
    .select('serial')
    .executeTakeFirst()
...
const validateUpdate = async (update: CabUpdate, cab: number | null) => {
  ...
  for (let [key, val] of Object.entries(update) as Entries<CabUpdate>) {
    ...
    try {
      ...
      if (val !== null)
        (update as any)[key] = ((await CAB_VALIDATORS.get(key)!(val, cab)) ??
          val) as any
      else (update as any)[key] = null
    } catch (e: any) {
...
export const updateMachine = async ({
  ...
  machine,
  update,
}: { arcade: number; machine: number; update: CabUpdate }) => {
  const user = await requireUser()
  const arcadePermissions = await getArcadePermissions(user, arcade)
...
export const createMachine = async ({
  arcade,
  update,
}: { arcade: number; update: CabUpdate }) => {
  const user = await requireUser()
  const arcadePermissions = await getArcadePermissions(user, arcade)
  ...
  return {
    ...
    data: structuredClone(
      await getArcadeCabs({ arcade, user, permissions: arcadePermissions }),
    ),
  }
}
