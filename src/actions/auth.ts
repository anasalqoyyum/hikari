...
export const requireUser = async (
  ...
  if (!user) {
    return redirect(
      `/auth/login?error=1&callbackUrl=${encodeURIComponent(
        headers().get('x-path') ?? '/',
      )}`,
    )
  }
...
export const login = async (options?: LoginOptions) => {
  if (!options)
    return {
      redirect: (await signIn(undefined, { redirect: false })) as string,
    }
...
export const register = async (formData: FormData) => {
  ...
  if (!accessCode) return { error: true, message: 'Access code required' }
  ...
  if (!EMAIL_REGEX.test(email)) return { error: true, message: 'Invalid email' }
    ...
    .where(({ eb, and, or, fn }) =>
      or([
        ...
        and([
          ...
          or([eb('u.username', 'is not', null), eb('u.email', 'is not', null)]),
        ]),
      ]),
    )
    .select('u.id')
  ...
  if (!user) return { error: true, message: 'Access code does not exist' }
