...
export const getFriendRequests = async () => {
  ...
  return structuredClone(
    await db
      .selectFrom('hikari_friend_requests as req')
      .where('user', '=', user.id)
      .innerJoin('aime_user as u', 'u.id', 'req.friend')
      .innerJoin('hikari_user_ext as ext', 'ext.userId', 'u.id')
      .select([
        'req.friend',
        'req.createdDate',
        'req.uuid as reqUuid',
...
export const sendFriendRequest = async (toUser: number) => {
  ...
  const existing = await db
    .selectFrom('hikari_friend_requests')
    .where('user', '=', toUser)
  ...
  await db
    .insertInto('hikari_friend_requests')
    .values({
      user: toUser,
...
export const unfriend = async (friend: number) => {
  ...
  await db.transaction().execute(async (trx) => {
    await trx
      .deleteFrom('hikari_user_friends')
      .where(({ or, eb, and }) =>
        or([
          and([eb('user1', '=', friend), eb('user2', '=', user.id)]),
          and([eb('user2', '=', friend), eb('user1', '=', user.id)]),
        ]),
      )
      .executeTakeFirst()
...
export const acceptFriendRequest = async (id: string) => {
  ...
  await db.transaction().execute(async (trx) => {
    const selectSql = withChuniRivalCount(trx)
      .with('insert_users', (db) =>
        db
          .selectNoFrom(({ lit }) => [
            lit(user.id).as('u1'),
            lit(request.friend).as('u2'),
          ])
          .union(
            db.selectNoFrom(({ lit }) => [
              lit(request.friend).as('u1'),
              lit(user.id).as('u2'),
            ]),
          ),
      )
      ...
      .select(
        ({ eb, fn, lit, selectFrom }) =>
          [
            'insert_users.u1 as user1',
            'insert_users.u2 as user2',
            eb(
              fn<number>('coalesce', [
                selectFrom('chuni_max_rival_count')
                  .whereRef(
                    'chuni_max_rival_count.user1',
                    '=',
                    'insert_users.u1',
                  )
                  .whereRef(
                    'chuni_max_rival_count.user2',
                    '=',
                    'insert_users.u2',
                  )
                  .select('maxRivalCount'),
                lit(0),
              ]),
              '<',
              lit(4),
    ...
    await trx
      .insertInto('hikari_user_friends')
      .columns(['user1', 'user2', 'chuniRival'])
      .expression(selectSql)
      .execute()

    await trx
      .deleteFrom('hikari_friend_requests')
      .where('uuid', '=', id)
      .executeTakeFirst()
...
const setRivalStatus = (
  user1: number,
  user2: number,
  chuniRival: number,
  trx: Transaction<GeneratedDB>,
) => {
  return trx
    .updateTable('hikari_user_friends')
    .where(({ and, eb, or }) =>
      or([
        and([eb('user1', '=', user1), eb('user2', '=', user2)]),
        and([eb('user2', '=', user1), eb('user1', '=', user2)]),
      ]),
    )
    .set({ chuniRival })
    .execute()
...
export const addFriendAsRival = async (friend: number) => {
  ...
  const rivalCount = await withChuniRivalCount()
    ...
    .where(({ or, eb }) =>
      or([eb('user', '=', user.id), eb('user', '=', friend)]),
    )
    .select(['user', 'rivalCount'])
    .execute()

  const userRivalCount = Number(
    rivalCount.find((r) => r.user === user.id)?.rivalCount ?? 0,
  )
  const friendRivalCount = Number(
    rivalCount.find((r) => r.user === friend)?.rivalCount ?? 0,
  )
  ...
  if (userRivalCount >= 4)
    return {
      error: true,
      message:
        'You already have 4 rivals. You must remove a rival before adding a new one.',
    }
  if (friendRivalCount >= 4)
    return {
      error: true,
      message:
        'This user already has 4 rivals. They must remove a rival before you can add them as a rival.',
    }
