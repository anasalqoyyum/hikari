...
type UseAudioEvents = Partial<{
  readonly [K in keyof HTMLMediaElementEventMap]:
...
export const useAudio: UseAudio = (srcOrEvents, eventsOrCb, cb?) => {
  ...
  const events =
    typeof eventsOrCb === 'object'
      ? eventsOrCb
      : (srcOrEvents as UseAudioEvents)
  const callback = useCallback(
    typeof cb === 'function' ? cb : (eventsOrCb as UseAudioCallback),
    [],
  )
  ...
  useEffect(() => {
    ...
    Object.entries(events).forEach(([name, listener]) => {
      ...
      else audio.addEventListener(name, ...(listener as [any, any]))
    })
    ...
    return () => {
      Object.entries(events).forEach(([name, listener]) => {
        ...
        else audio.removeEventListener(name, ...(listener as [any, any]))
      })
      audio.pause()
    }
  }, [src])
