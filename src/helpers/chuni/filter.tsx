...
export const CHUNI_FILTER_DIFFICULTY: FilterField<
  ChuniMusic,
  'select',
  'difficulty'
> = {
  ...
  filter: (val: Set<string>, data) =>
    !val.size || val.has(data.chartId?.toString()!),
}
...
export const CHUNI_FILTER_LAMP: FilterField<ChuniMusic, 'select', 'lamp'> = {
  ...
  filter: (val: Set<string>, data) => {
    ...
    if (
      ...
      return false
    else if (val.has('aj') && !val.has('fc') && !data.isAllJustice) return false
    else if (val.has('fc') && !data.isFullCombo) return false
    return true
  },
}

export const CHUNI_FILTER_WORLDS_END_TAG: FilterField<
  ChuniMusic,
  'select',
  'worldsEndTag'
> = {
  ...
  value: new Set<string>(),
  className:
    'col-span-6 md:col-span-3 lg:col-span-2 xl:col-span-2 5xl:col-span-1',
  props: {
    children: CHUNI_WORLDS_END_TAGS.map((t) => (
  ...
  filter: (val: Set<string>, data) =>
    !val.size || !data.worldsEndTag || val.has(data.worldsEndTag),
}
...
export const CHUNI_FILTER_SCORE: FilterField<ChuniMusic, 'select', 'score'> = {
  ...
  value: new Set<string>(),
  className:
    'col-span-6 sm:col-span-6 md:col-span-3 lg:col-span-2 xl:col-span-2 2xl:col-span-3 5xl:col-span-1',
  props: {
    children: CHUNI_SCORE_RANKS.map((s, i) => (
      <SelectItem key={i.toString()} value={i.toString()}>
        {s}
      </SelectItem>
    )).reverse(),
  ...
  filter: (val: Set<string>, data) =>
    !val.size || val.has(data.scoreRank?.toString()!),
}

export const CHUNI_FILTER_FAVORITE: FilterField<
  ChuniMusic,
  'switch',
  'favorite'
> = {
  ...
  value: false,
  className:
    'justify-self-end col-span-6 md:col-span-3 md:col-start-10 lg:col-span-2 lg:col-start-auto 2xl:col-span-1 5xl:col-start-12',
...
> = {
  ...
  value: [1, 5],
  className:
    'px-2 col-span-full md:col-span-6 md:col-start-4 md:row-start-2 lg:row-start-auto lg:col-start-auto lg:col-span-4 5xl:col-span-2 5xl:row-start-1 5xl:col-start-6',
  ...
  props: {
    ...
    getValue: (v) =>
      Array.isArray(v)
        ? `${worldsEndStars(v[0])}\u2013${worldsEndStars(v[1])}`
        : worldsEndStars(v),
    renderValue: ({ children, className, ...props }: any) => (
      <span className="text-[0.65rem]" {...props}>
        {children}
      </span>
    ),
  },
...
export const CHUNI_FILTER_LEVEL: FilterField<ChuniMusic, 'slider', 'level'> = {
  ...
  value: [0, 90],
  className:
    'px-2 col-span-full md:col-span-6 lg:col-span-4 5xl:col-span-2 5xl:row-start-1 5xl:col-start-8',
  filter: ([a, b]: number[], val) => {
    if (val.worldsEndTag) return true
  ...
  props: {
    ...
    getValue: (v) =>
      Array.isArray(v)
        ? `${getLevelFromStop(v[0])}\u2013${getLevelFromStop(v[1])}`
        : getLevelFromStop(v).toString(),
  },
}
...
export const CHUNI_FILTER_RATING: FilterField<ChuniMusic, 'slider', 'rating'> =
  {
    ...
    value: [0, 17.55],
    className:
      'px-2 col-span-full md:col-span-6 lg:col-span-4 5xl:col-span-2 5xl:row-start-1 5xl:col-start-10',
    filter: ([a, b]: number[], val) => {
      if (val.worldsEndTag) return true
