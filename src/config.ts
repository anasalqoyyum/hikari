...
type ConfigEntry<T extends keyof GlobalConfig> = {
  ...
  validate: (
    ...
    | { error?: false; value?: GlobalConfig[T] }
    | undefined
    | void
}
...
export const getGlobalConfig: GetConfig = <T extends keyof GlobalConfig>(
  key?: T,
...
export const setGlobalConfig = async (update: Partial<GlobalConfig>) => {
  for (const [key, value] of Object.entries(update)) {
    ...
    if (val === (CONFIG as any)[key]) delete update[key as keyof typeof update]
    else (CONFIG as any)[key] = res?.value ?? value
  }
  ...
  await db.transaction().execute(async (trx) => {
    for (const [key, value] of Object.entries(update)) {
      await trx
        .updateTable('hikari_global_config')
        .where('key', '=', key)
...
export const loadConfig = async () => {
  const entries = await db
    .selectFrom('hikari_global_config')
    .selectAll()
    .execute()
  ...
  if (!entries.length) {
    console.log(
      '[INFO] first startup detected, loading global config default values',
    )
    CONFIG = Object.fromEntries(
      Object.entries(CONFIG_ENTRIES).map(([k, { defaultValue }]) => {
    ...
    CONFIG = Object.fromEntries(
      Object.entries(CONFIG_ENTRIES).map(([k, { defaultValue, validate }]) => {
        ...
        try {
          ...
          console.warn(
            `[WARN] failed to parse config value for ${k}, falling back to default`,
          )
        ...
        if (res?.error) {
          console.warn(
            `[WARN] failed to parse config value for ${k}: ${
  ...
  await db.transaction().execute(async (trx) => {
    if (inserts.length)
      await trx.insertInto('hikari_global_config').values(inserts).execute()
