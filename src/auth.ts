...
import { createHikariTeamsFromExistingTeams } from './data/team'
import { createHikariFriendsFromExistingFriends } from './data/friend'

let basePath = process.env.BASE_PATH ?? ''
if (basePath.endsWith('/')) basePath = basePath.slice(0, -1)

const selectUserProps = (
  builder: SelectQueryBuilder<GeneratedDB & { u: AimeUser }, 'u', {}>,
) =>
  builder
    .leftJoin(
      (eb) =>
        eb
          .selectFrom('chuni_profile_data as chuni')
          .where(({ eb, selectFrom }) =>
            eb(
              'chuni.version',
              '=',
    ...
    .leftJoin('hikari_user_ext as ext', 'ext.userId', 'u.id')
    .select(
      ({ fn }) =>
        [
          'u.username',
...
if (['0', 'false', 'no'].includes(process.env.COOKIE_SECURE?.toLowerCase()!))
  ...
  ['1', 'true', 'yes'].includes(process.env.COOKIE_SECURE?.toLowerCase()!)
)
...
const nextAuth = NextAuth({
  ...
  callbacks: {
    async jwt({ token, user }) {
      ...
      if (db) {
        const dbUser = await selectUserProps(
          db
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize({ username, password }, req) {
        ...
        const user = await selectUserProps(
          db
            .selectFrom('aime_user as u')
            .where(({ eb, fn }) =>
              eb(
                fn<string>('lower', ['u.username']),
                '=',
        ...
        const { password: _, ...payload } = user satisfies {
          [K in keyof DBUserPayload]: DBUserPayload[K] | null
        }
...
export const auth =
  process.env.NEXT_RUNTIME !== 'edge' ? cache(nextAuth.auth) : (null as never)
