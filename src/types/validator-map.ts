...
type Validator<T, K extends keyof T, D> = undefined extends D
  ? (
      val: NonNullable<Required<T>[K]>,
    ) => Awaitable<T[K] | null | undefined> | void
  : (
      val: NonNullable<Required<T>[K]>,
      data: D,
    ) => Awaitable<T[K] | null | undefined> | void
...
type ValidatorChecker<T, R, D = undefined> = undefined extends D
  ? (val: T) => Promise<ValidationResult<R>>
...
type Validated<
  T,
  NonNullableKeys extends keyof T,
  RequiredKeys extends keyof T,
> = { [K in keyof T]?: K extends NonNullableKeys ? T[K] : T[K] | null } & {
...
> {
  ...
  withValidator<K extends keyof T>(
    key: K,
    validator: Validator<Required<ValidationInput<T>>, K, D>,
  ): this {
  ...
  validate = (async (value, data) => {
    for (const k of this._requiredKeys) {
      if (!(k in value)) return { error: true, message: `Key ${k} is required` }
    }

    const update: ValidationInput<T> = {}

    for (let [key, val] of Object.entries(value) as Entries<
      ValidationInput<T>
    >) {
      ...
      try {
        if (val !== null)
          val =
            (await this.validatorMap.get(key as any)!(val as any, data)) ?? val
      } catch (e: any) {
    ...
    ValidationInput<T>,
    Validated<T, NonNullableKeys, RequiredKeys>,
    D
  >
}
