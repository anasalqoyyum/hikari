...
export const PermissionEditModal = ({
  user,
  onClose,
  permissions,
  displayUpTo,
  ...
  return (
    <Modal onClose={onModalClose} isOpen={user !== null}>
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader>
            ...
            <ModalBody>
              {[...permissions]
                .filter(([p]) => p <= (displayUpTo ?? Infinity))
                .map(([permission, { description, title }]) => (
                  <div key={permission} className="flex gap-2 items-center">
                    <Checkbox
                      size="lg"
                      isSelected={!!(editingPermissions & (1 << permission))}
                      isDisabled={
                        permission === UserPermissions.OWNER &&
                        disallowDemoteOwners &&
                        !!(user?.permissions! & (1 << UserPermissions.OWNER))
                      }
                    ...
                    <Tooltip
                      content={
                        permission === UserPermissions.OWNER &&
                        disallowDemoteOwners
                          ? `${description} (owners cannot be removed from the owner role)`
                          : description
                      }
                    >
              ...
              <Button
                ...
                onPress={() => {
                  ...
                  if (
                    disallowDemoteOwners &&
                    !(user?.permissions! & (1 << UserPermissions.OWNER)) &&
                    editingPermissions & (1 << UserPermissions.OWNER)
                  ) {
                    setTimeout(() => {
                      confirm(
                        'Once a user is promoted to owner, they cannot be removed from the owner role.',
