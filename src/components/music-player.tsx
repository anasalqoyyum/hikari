...
export const MusicPlayer = ({
  ...
  const audioRef = useAudio(
    ...
    {
      loadedmetadata() {
        if (this.duration !== undefined) setDuration(this.duration)
        setProgress(0)
      },
      timeupdate() {
        if (this.currentTime !== undefined) setProgress(this.currentTime)
      },
  ...
  return (
    <Card
      isBlurred
      radius="none"
      className={`border-none shadow-lg sm:rounded-2xl w-full max-w-full sm:max-w-[48rem] ${className ?? ''}`}
    >
