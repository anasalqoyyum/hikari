...
const SelectModalList = <D extends Data, I extends keyof D>({
  onSelectionChanged,
  setSelected,
  gap,
  rowSize,
  ...
  SelectModalProps<'list', D, I>,
  'itemId' | 'onSelectionChanged' | 'gap' | 'rowSize' | 'renderItem' | 'items'
> & { selected?: D | null; setSelected: (d: D) => void }) => {
  ...
  return (
    <div ref={listRef} className="w-full overflow-y-auto overflow-x-hidden">
      <div
...
const SelectModalGrid = <D extends Data, I extends keyof D>({
  onSelectionChanged,
  setSelected,
  gap,
  rowSize,
      ...
      >
        {virtualizer.getVirtualItems().map((item) => {
          const row = items.slice(
            item.index * itemsPerRow,
            (item.index + 1) * itemsPerRow,
          )
          const children = row.map((item, i) => (
            <Button
              key={i}
              style={buttonStyle}
              className={`w-full px-0 h-full ${selected?.[itemId] === item[itemId] ? 'bg-gray-400/75' : 'bg-transparent'}`}
              onPress={() => {
          ...
          return (
            ...
            >
              {children}
              {item.index === rowCount - 1
                ? [...new Array(itemsPerRow - children.length)].map((_, i) => (
                    <div
                      key={i}
                      style={buttonStyle}
...
>({
  ...
  const filter = useDebounceCallback((query: string) => {
    ...
    setFilteredItems(
      items.filter(({ name }) => name?.toLowerCase().includes(lowerQuery)),
    )
  }, 100)
...
export const SelectModalButton = <
  T extends 'grid' | 'list',
  D extends Data,
  I extends keyof D,
>(
  ...
  useEffect(() => {
    if (window.location.hash === `#modal-${modalId}`) setOpen(true)
  }, [])
