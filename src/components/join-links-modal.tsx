...
export const JoinLinksModal = ({
  ...
  useEffect(() => {
    setFullPrefix(
      window.location.origin + process.env.NEXT_PUBLIC_BASE_PATH + prefix,
    )
  }, [prefix])
        ...
        {(onClose) => (
          <>
            ...
            <ModalBody className="flex flex-col overflow-y-auto gap-0">
                  ...
                  <Button
                    ...
                    onPress={() => {
                      ...
                      onCreate(uses)
                        .then((id) =>
                          setLinks((l) => [
                            { id, remainingUses: uses, totalUses: 0 },
                            ...l,
                          ]),
                        )
              ...
              {!links.length && (
                <div className="italic text-gray-500">No links</div>
              )}
              {links.map((link) => (
                <Fragment key={link.id}>
                  <div className="flex items-center gap-x-0.5">
                    <Link
                      href={fullPrefix + link.id}
                      prefetch={false}
                    ...
                    <Tooltip content="Copy link">
                      <Button
                        ...
                        className="ml-auto"
                        onPress={() =>
                          navigator?.clipboard?.writeText(fullPrefix + link.id)
                        }
                      >
                    ...
                    <Tooltip
                      content={<span className="text-danger">Delete link</span>}
                    >
                      <Button
                        isIconOnly
                        variant="light"
