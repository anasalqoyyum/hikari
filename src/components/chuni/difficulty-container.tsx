...
export const ChuniDifficultyContainer = ({
  children,
  className,
  difficulty,
  containerClassName,
  ...props
}: ChuniDifficultyContainerProps) => {
  return (
    <div className={`relative ${className ?? ''}`} {...props}>
      {BACKGROUNDS[difficulty].map((className, i) => (
        <div
          className={`${className} w-full h-full absolute inset-0 z-0 rounded`}
          key={i}
        />
      ))}
      <div className={`z-0 relative w-full h-full ${containerClassName ?? ''}`}>
        {children}
      </div>
    </div>
  )
