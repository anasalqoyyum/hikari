...
import {
  CHUNI_SCORE_RANKS,
  CHUNI_SCORE_THRESHOLDS,
  getRankFromScore,
} from '@/helpers/chuni/score-ranks'
...
const calculateTop = ({
  ...
  topRating?: ChuniUserRating
  subtract: BigDecimal | null
  rating: BigDecimal
  totalTopRating: BigDecimal
}) => {
  ...
  if (subtract !== null && rating.compare(subtract) > 0) {
    topIncrease = totalTopRating
      .add(rating.sub(subtract))
      .div(30, 4n)
      .sub(totalTopRating.div(30, 4n))
      .toFixed(3)
    topIndex = topRating.top.findIndex((t) => rating.compare(t.rating) >= 0)
    if (topIndex === -1) topIndex = null
  }

  return [topIncrease, topIndex] as const
}

type MissAttackState = {
  miss: string
  attack: string
  totalAttack: number
  missPercent: number
}
type MissAttackAction =
  | { miss: string }
  | { attack: string; recompute?: boolean }
  | { miss: string; totalAttack: number }
  | { missPercent: number }
  | 'recompute'
type NoteValues = {
  noteMiss: string
...
export const ChuniScoreCalculator = ({
  topRating,
  music,
  onClose,
}: ChuniScoreCalculatorProps) => {
  ...
  const [{ miss, attack, missPercent }, dispatchScore] = useReducer(
    (state: MissAttackState, action: MissAttackAction): MissAttackState => {
      if (action === 'recompute') {
      ...
      if ('miss' in action) {
        ...
        const miss = Math.floor(
          Math.max(Math.min(+action.miss, totalAttack / 2), 0),
        )
        ...
        let attack = Math.floor(
          Math.max(Math.min(+action.attack, totalAttack), 0),
        )
        const miss = Math.min(
          (attack < +state.attack && action.recompute ? Math.ceil : Math.floor)(
            (totalAttack - attack) / 2,
          ),
          Math.floor(totalAttack / 2),
        )
        if (action.recompute) attack = state.totalAttack - miss * 2
  ...
  const [noteCount, dispatchNote] = useReducer(
    (state: NoteState, action: RequireExactlyOne<NoteValues> | number) => {
      if (typeof action === 'number')
        return {
          ...
          noteAttack: '0',
          noteJustice: '0',
          noteJusticeCritical: action.toString(),
          totalNote: action,
        }
      ...
      if (state[key] !== '' && Number.isFinite(+state[key]))
        state[key] = Math.floor(
          Math.min(Math.max(+state[key], 0), state.totalNote),
        ).toString()

      if (
        state.order.some((k) => state[k] === '' || !Number.isInteger(+state[k]))
      )
        return state

      let sum = state.order.reduce((t, k) => t + +state[k], 0)
      if (sum === state.totalNote) return state
    ...
    },
    {
      ...
      noteAttack: '0',
      noteJustice: '0',
      noteJusticeCritical: '0',
      totalNote: 0,
      order: ['noteMiss', 'noteAttack', 'noteJustice', 'noteJusticeCritical'],
    },
  )

  const totalTopRating = useMemo(
    () =>
      topRating?.top.reduce((t, x) => t.add(x.rating), BigDecimal.ZERO) ??
      BigDecimal.ZERO,
    [topRating],
  )
  const musicLevel = useMemo(
    () => (music ? new BigDecimal(music.level?.toFixed(1)!) : null),
    [music],
  )
  const subtract = useMemo(() => {
    if (!music) return null
    const top = topRating?.top.find(
      (r) => r.songId === music.songId && r.chartId === music.chartId,
    )
    const maxPossibleRating = musicLevel!.add('2.15')

    const smallestRating = topRating?.top.at(-1)?.rating ?? 0
    if (maxPossibleRating.compare(smallestRating) <= 0) return null

    if (top) {
      const rating = new BigDecimal(top.rating)
      const currentRatingDiff = rating.sub(musicLevel!)
      if (currentRatingDiff.compare('2.15') >= 0) return null
      return rating
    } else {
  ...
  const maxPossibleTopIncrease = useMemo(() => {
    ...
    return totalTopRating
      .add(maxPossibleRating.sub(subtract))
      .div(30, 4n)
      .sub(totalTopRating.div(30, 4n))
      .toString()
  ...
  const {
    ...
    const data: {
      ...
      topIndex: null,
      targetTopScore: null,
      targetTotalRating: null,
      computedNoteScore: null,
      computedRating: null,
    }
    ...
    if (selectedKey === 'rating') {
      ...
      data.musicRating = computedRating = chuniRating(
        Math.max(Math.min(Math.floor(+score), 1010000), 0),
        music?.level ?? 0,
        5n,
      )
      ...
      data.computedNoteScore = Math.floor(
        (+noteCount.noteJusticeCritical * 1_010_000 +
          +noteCount.noteJustice * 1_000_000 +
          +noteCount.noteAttack * 500_000) /
          music.allJudgeCount,
      )
      computedRating = chuniRating(
        data.computedNoteScore,
        music?.level ?? 0,
        5n,
      )
    }
    ...
    if (computedRating !== null) {
      ...
      const [topIncrease, topIndex] = calculateTop({
        topRating,
        subtract,
        totalTopRating,
  ...
  useEffect(() => {
    if (music) dispatchNote(music.allJudgeCount)
  }, [music, dispatchNote])

  let targetScore: number | null
  if (selectedKey === 'rating')
    targetScore = Math.max(Math.min(Math.floor(+score), 1010000), 0)
  else if (selectedKey === 'score') targetScore = reverseScore
  else if (selectedKey === 'top') targetScore = targetTopScore
  else targetScore = computedNoteScore
  const validScore =
    targetScore !== null && !isNaN(+targetScore) && isFinite(targetScore)
  ...
  const badge = useMemo(
    () =>
      rank !== null && (
        <ChuniScoreRankBadge
          className="h-5 sm:h-8 ml-auto my-auto"
          rank={rank}
        />
      ),
    [rank],
  )
  const badgeDropdown = useMemo(
    () =>
      badge !== null &&
      selectedKey !== 'top' && (
        <Dropdown className="!min-w-0">
          <DropdownTrigger>
            <div className="ml-auto my-auto">{badge}</div>
          </DropdownTrigger>
          <DropdownMenu>
            {CHUNI_SCORE_RANKS.map((name, i) => (
              <DropdownItem
                key={i}
                ...
                onPress={() => {
                  ...
                  else if (i === 1) setRating('0.01')
                  else
                    setRating(
                      (
                        Math.ceil(
                          chuniRating(
                            CHUNI_SCORE_THRESHOLDS[i],
                            music?.level!,
                            3n,
                          )
                            .mul(100)
  ...
  useValueChange(
    () => {
      if (!music || targetScore === null || !Number.isInteger(targetScore))
        return
      ...
      const maxAttackCount =
        targetScore <= 500000
          ? noteCount
          : Math.floor(
              (noteCount * (10000 * justiceRatio - targetScore + 1000000)) /
                (10000 * (justiceRatio + 50)),
            )
      ...
      const justiceCriticalCount = Math.floor(
        remainingJusticeCount * justiceRatio,
      )
      const justiceCount = remainingJusticeCount - justiceCriticalCount

      setJusticeCount(justiceCount)

      const miss = Math.ceil(
        Math.min(maxAttackCount * missPercent, Math.floor(maxAttackCount / 2)),
      )
      const attack = maxAttackCount - 2 * miss
      dispatchScore({
        miss: miss.toString(),
        attack: attack.toString(),
        totalAttack: maxAttackCount,
      })
    },
    [music, targetScore, ratio],
    [miss, attack],
  )
  ...
  const scoreTargetText = useMemo(() => {
    ...
    if (selectedKey === 'score') score = reverseScore
    else if (selectedKey === 'top') score = targetTopScore
    else score = computedNoteScore

    return (
      <span className="pt-0.5">
        <span className="sm:text-lg text-xs">Score: </span>
        {score === Infinity ? (
  ...
  const topIncreaseText = topIncrease && (
    <span className="text-xs sm:text-sm mt-auto pb-0.5 sm:pb-1 ml-2.5">
      (Top +{topIncrease}
      {topIndex !== null && `, #${topIndex + 1}`})
    </span>
  )

  return (
    <Modal isOpen={music !== null} onClose={onModalClose} size="3xl">
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader>Score Calculator</ModalHeader>
            <ModalBody>
              <header className="font-semibold text-lg flex">
                {music?.title}{' '}
                <span className="ml-auto text-right">
                  {CHUNI_DIFFICULTIES[music?.chartId!]} ({music?.level}){' '}
                </span>
              </header>
              <div className="flex text-xs sm:text-sm md:text-medium">
                {music?.scoreMax !== null && (
                  <>
                    ...
                    {music?.scoreMax?.toLocaleString()} (
                    {floorToDp(+music?.rating!, 4)})
                  </>
                )}

                <span className="ml-auto text-right">
                  Note count: {music?.allJudgeCount}
                </span>
              </div>
              <Tabs
                fullWidth
              ...
              >
                <Tab key="rating" title="Score to Rating" className="p-0 m-0">
                  ...
                  />
                  {musicRating && (
                    <div className="sm:text-lg flex mt-2.5 flex-wrap">
                      <span className="mt-auto text-sm sm:text-lg">
                        Song rating:&nbsp;
                      </span>
                      <ChuniRating
                        rating={musicRating.mul(100).valueOf()}
                        className="inline text-lg max-sm:-mb-1 sm:text-2xl mt-auto"
                      >
                        {musicRating.toString()}
                      </ChuniRating>
                      {topIncreaseText}
                      {badgeDropdown}
                    </div>
                ...
                <Tab key="note" title="Note Count">
                  <div className="flex w-full justify-around text-xs sm:text-sm md:text-medium gap-x-2 items-end">
                    {(
                      ...
                      <span
                        key={key}
                        className={`${className} drop-shadow text-center max-sm:flex-1`}
                      >
                        {name}&nbsp;
                        <Input
                          className="max-sm:mt-1 inline-block max-w-[80px]"
                          size="sm"
                          type="number"
                          inputMode="numeric"
                          min="0"
                          max={music?.allJudgeCount}
                          classNames={{ input: '!text-inherit' }}
                          value={noteCount[key]}
                          onValueChange={(v) =>
                            dispatchNote({ [key]: v } as any)
                          }
                        />
                      </span>
                    ))}
                  ...
                  <div className="mt-2.5 flex items-center flex-wrap">
                    ...
                    {computedRating !== null && (
                      <div className="w-full flex">
                        <span className="text-xs sm:text-sm md:text-medium self-center">
                          Rating:&nbsp;
                        </span>
                        <ChuniRating
                          rating={computedRating.mul(100).valueOf()}
                  ...
                  {maxPossibleTopIncrease === null ? (
                    ...
                    <div>
                      ...
                      <Input
                        labelPlacement="outside-left"
                        placeholder="Enter rating"
                        label="Top rating increase"
                        type="number"
                        inputMode="numeric"
                        min="0"
                        max={maxPossibleTopIncrease}
                        step="0.01"
                        size="md"
                        classNames={{
                          mainWrapper: 'w-full',
                          label: 'text-nowrap mr-1.5',
                        }}
                      ...
                      <div className="mt-2.5 flex items-center">
                        {scoreTargetText}
                        {targetTotalRating !== null && (
                          <span className="max-sm:mt-0.5 text-xs sm:text-sm md:text-medium">
                            &nbsp;(Top rating{' '}
                            <ChuniRating
                              className="inline-block text-sm md:text-lg"
                              rating={targetTotalRating.mul(100)?.valueOf()!}
                            >
              ...
              {validScore && selectedKey !== 'note' && (
                <>
                  ...
                  <section className="text-xs md:text-sm lg:text-medium flex gap-y-2 flex-wrap items-center">
                    ...
                    <span className="dark:text-yellow-400 text-yellow-500 drop-shadow">
                      Justice&nbsp;
                    </span>
                    <span className="dark:text-yellow-400 text-yellow-500 drop-shadow">
                      Critical&nbsp;
                    ...
                    {unachievable ? (
                      ...
                      <>
                        ...
                        <div className="flex gap-6 max-sm:w-full max-sm:mx-auto ml-auto items-center justify-center">
                          ...
                          <div>
                            <Input
                              className="inline-block w-20 mr-1"
                              size="sm"
                              inputMode="numeric"
                              type="number"
                              min="0"
                              step="1"
                              classNames={{ input: '!text-emerald-600' }}
                            ...
                            />
                            <span className="text-emerald-600">Attack</span>
                          </div>
                          <div>
                            <Input
                              className="inline-block w-20 mr-1"
                              ...
                              value={miss.toString()}
                              onValueChange={(v) => dispatchScore({ miss: v })}
                            />
