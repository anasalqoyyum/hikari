...
const ALLCHARS = new Set([
  ...`　＃＆＊．＠＋－＝：；？！～／ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚ０１２３４５６７８９${SYMBOLS}`,
])

const FULLWIDTHS = new Map([...ALLCHARS].map((c) => [c.normalize('NFKC'), c]))
...
export const ChuniNameModal = ({
  ...
  if ([...editingUsername].length > 8)
    warnings.push(
      'Your username is longer than 8 characters. It may be squished in-game.',
    )
  if ([...editingUsername].some((c) => !ALLCHARS.has(c)))
    warnings.push(
      'Your username contains characters not typically used in-game. It may not show correctly.',
    )

  useEffect(() => {
    if (isOpen && !previousOpen.current) setEditingUsername(username)
    previousOpen.current = isOpen
  }, [isOpen, username, previousOpen])

  return (
    <Modal size="3xl" isOpen={isOpen} onClose={onModalClose}>
      <ModalContent>
        {(onModalClose) => (
          <>
            <ModalBody className="max-sm:px-2">
              ...
              {!editingUsername.length && (
                <div className="flex text-danger mb-2 items-center max-sm:text-sm">
              ...
              {warnings.map((warning) => (
                <div
                  key={warning}
                  className="flex text-warning mb-2 items-center max-sm:text-sm"
                >
              ...
              <Input
                ...
                classNames={{
                  input: `[font-feature-settings:"fwid"] font-semibold`,
                }}
              />
                  ...
                  <div
                    ...
                    onClick={(e) => {
                      ...
                      setEditingUsername((val) => {
                        if (!inputRef.current || [...val].length >= 25)
                          return val
                        const chars = [...val]
                        ...
                        return (
                          chars.slice(0, selectionStart!).join('') +
                          s +
                          chars.slice(selectionEnd!).join('')
                        )
                      })
                    }}
                  >
