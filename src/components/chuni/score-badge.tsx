...
type Variant =
  | keyof typeof ChuniScoreBadgeVariant
  | (typeof ChuniScoreBadgeVariant)[keyof typeof ChuniScoreBadgeVariant]
...
export const ChuniScoreBadge = ({
  children,
  variant,
  className,
  fontSize,
}: ChuniScoreBadgeProps) => {
  const size = sizes[fontSize ?? 'md']

  return (
    <div className={`aspect-[72/16] font-helvetica ${className ?? ''}`}>
      <div className="@container-size w-full h-full text-black">
        <div
          className={`${BACKGROUNDS[typeof variant === 'number' ? variant : ChuniScoreBadgeVariant[variant]]} w-full h-full flex items-center justify-center border-black border`}
        >
          <span className={`font-bold drop-shadow-[0_0_25cqh_#fff] ${size}`}>
            {children}
          </span>
        </div>
      </div>
...
export const ChuniLampSuccessBadge = ({
  success,
  className,
}: { className?: string; success: number }) => {
  const text = CHUNI_LAMPS.get(success)?.toUpperCase()
  const fontSize = text?.length! > 5 ? (text?.length! > 10 ? 'xs' : 'sm') : 'md'
  return (
    <ChuniScoreBadge
      variant={getVariantFromLamp(success)}
      className={className}
      fontSize={fontSize}
    >
      <span
        className={
          fontSize === 'md' ? 'tracking-[1.5cqw]' : 'tracking-[0.75cqw]'
        }
      >
        {text}
      </span>
...
export const ChuniLampComboBadge = ({
  className,
  isFullCombo,
  isAllJustice,
}: {
  ...
  return (
    <ChuniScoreBadge
      variant={isAllJustice ? 'platinum' : 'gold'}
      className={className}
      fontSize="sm"
    >
...
export const ChuniScoreRankBadge = ({
  rank,
  ...rest
}: { rank: number } & Pick<ChuniScoreBadgeProps, 'className' | 'fontSize'>) => {
  const scoreRank = CHUNI_SCORE_RANKS[rank]
  return (
    <ChuniScoreBadge variant={getVariantFromRank(rank)} {...rest}>
