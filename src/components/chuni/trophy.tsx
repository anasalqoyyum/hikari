...
const TROPHY_Y = [6, 81, 156, 231, 306, 381, 456, 531, 606].map(
  (x) => (x / 1024) * 100,
)
...
export const ChuniTrophy = ({ name, rarity, className }: ChuniTrophyProps) => {
  return (
    <div
      className={`w-full overflow-hidden relative aspect-[593/62] @container-size ${className}`}
    >
      <div
        className="z-10 absolute w-full h-full flex items-center justify-center top-0"
        lang="ja"
      >
        <div
          className="w-11/12 pb-[2.5cqh] leading-loose text-black font-semibold text-center text-nowrap overflow-hidden text-[50cqh]"
          title={name ?? 'Trophy'}
        >
          {name}
        </div>
      ...
      <div className="w-[129.51%] aspect-[768/1024] relative">
        <Image
          ...
          alt={name ?? 'Trophy'}
          title={name ?? 'Trophy'}
          className="w-full absolute h-auto left-[-0.78125%]"
          style={{
