...
type PromptOptions = { title: string; size?: ModalProps['size'] } & (
  | ({ message: string; content?: never } & Partial<
      Pick<InputProps, 'type' | 'name' | 'label' | 'placeholder'>
    >)
  | {
      content: (value: string, setValue: (v: string) => void) => ReactNode
      message?: never
    }
)
...
export const PromptProvider = ({ children }: { children: ReactNode }) => {
  ...
  const setPrompt: PromptCallback = useCallback(
