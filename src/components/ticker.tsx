...
export const TickerHoverProvider = ({ children }: TickerHoverProviderProps) => {
  ...
  return (
    <TickerHoverContext.Provider value={hovering}>
...
export const Ticker = ({
  ...
  const outerAnimation = noDelay
    ? 'animate-[outer-overflow-nodelay_10s_linear_infinite_alternate]'
    : 'animate-[outer-overflow_10s_linear_infinite_alternate]'
  const innerAnimation = noDelay
    ? 'animate-[inner-overflow-nodelay_10s_linear_infinite_alternate]'
    : 'animate-[inner-overflow_10s_linear_infinite_alternate]'
  ...
  const hoverClass =
    !hoverOnly && hovering ? '[&_*]:[animation-play-state:paused]' : ''

  if (hoverOnly && !hovering)
    return (
      <div
        className={`text-nowrap whitespace-nowrap overflow-hidden w-full ${className ?? ''}`}
  ...
  return (
    <div
      className={`text-nowrap whitespace-nowrap overflow-hidden w-full ${hoverClass} ${className ?? ''}`}
