...
export type Filterers<D, N extends string> = readonly FilterField<
  D,
  keyof FilterTypes,
  N
>[]
...
const getParamsFromState = (data: LocalState & { query: string }) => {
  ...
  Object.entries(data).forEach(([key, val]) => {
    ...
    if (
      ...
      if (val.from) params.set(`${key}-begin`, val.from.toISOString())
      if (val.to) params.set(`${key}-end`, val.to.toISOString())
    }
  })
...
const getStateFromParams = <K extends keyof LocalState & string>(
  ...
  if (typeof val === 'boolean') {
    ...
    if (params.has(key)) return params.get(key)!.split(',')
  } else if (params.has(`${key}-begin`) || params.has(`${key}-end`)) {
    const range: DateRange = {
      from: new Date(params.get(`${key}-begin`)!),
    }
    if (params.has(`${key}-end`)) range.to = new Date(params.get(`${key}-end`)!)
    return range
  }
...
const getFilterStateFromParams = <S extends object>(
  params: ReadonlyURLSearchParams,
  state: S,
) =>
  Object.fromEntries(
    (Object.entries(state) as Entries<S>).map(([key, val]) => {
      return [key, getStateFromParams(params, key as any, val)]
    }),
  ) as S

const FilterSorterComponent = <
  D,
  M extends string,
  N extends string,
  S extends string,
>({
  ...
  className,
  children,
}: FilterSorterProps<D, M, N, S> & { defaultData: any }) => {
  ...
  const [filterState, _setFilterState] = useState<typeof defaultFilterState>(
    getFilterStateFromParams(
      params,
      Object.keys(payloadFilterState).length
        ? payloadFilterState
        : defaultFilterState,
    ),
  )
  const [pageSize, _setPageSize] = useState<Set<string>>(
    getStateFromParams(
      params,
      'pageSize',
      defaultPageSize ?? new Set([(pageSizes?.[0] ?? 25).toString()]),
    )!,
  )
  const [sorter, _setSorter] = useState<Set<string>>(
    getStateFromParams(
      params,
      'sorter',
      defaultSorter ?? new Set(sorters.length ? [sorters[0].name] : []),
    )!,
  )
  const [ascending, _setAscending] = useState<boolean>(
    getStateFromParams(
      params,
      'ascending',
      storedAscending ?? defaultAscending ?? true,
    )!,
  )
  const [displayMode, _setDisplayMode] = useState<Set<string>>(
    getStateFromParams(
      params,
      'displayMode',
      defaultDisplayMode ??
        new Set([displayModes?.length ? displayModes[0].name : '']),
    )!,
  )
  const [currentPage, _setCurrentPage] = useState(
    getStateFromParams(params, 'currentPage', 1)!,
  )
  ...
  const onChange = useDebounceCallback(
    useCallback(() => {
      ...
      if (resetPage.current) {
        ...
        page = 1
        if (dataRemote) setTotalCount(-1)
      }

      const newParams = getParamsFromState({
        sorter,
        ascending,
        pageSize,
        currentPage: page,
      ...
      if (Array.isArray(dataDep)) {
        ...
        const filteredSorted = dataDep
          .filter(
            (d) =>
              filterers.every((f) => f.filter?.(filterState[f.name], d)) &&
              searcher?.(lower, d)!,
          )
          .sort(sorters.find((s) => sorter.has(s.name))!.sort)
        if (ascending) setProcessedData(filteredSorted)
        else setProcessedData(filteredSorted.reverse())
        setTotalCount(filteredSorted.length)

        if (
          !Number.isNaN(pageSizeNum) &&
          currentPage > Math.ceil(filteredSorted.length / pageSizeNum)
        )
          setCurrentPage(1)
  ...
  useEffect(() => {
    if (params.toString() === lastParams.current) return
  ...
  const updateLocalState = (
    payload: Partial<LocalState & { query: string }>,
  ) => {
    const state = {
      sorter,
      ascending,
    ...
    const data = JSON.stringify(state, (k, v) =>
      v instanceof Set ? { type: 'set', value: [...v] } : v,
    )
  ...
  const setPageSize = (size: typeof pageSize) => {
    ...
    const newPageNum = Number.isNaN(sizeNum)
      ? 1
      : Math.floor((pageSizeNum * (currentPage - 1)) / sizeNum) + 1
  ...
  const setFilterState = (
    s: typeof filterState | ((s: typeof filterState) => typeof filterState),
  ) => {
  ...
  const renderedData = useMemo(
    () => {
      ...
      const pageData =
        dataRemote || Number.isNaN(pageSizeNum)
          ? processedData
          : processedData.slice(
              pageSizeNum * (currentPage - 1),
              pageSizeNum * currentPage,
            )

      if (!pageData.length)
        return (
          <div className="m-auto text-lg text-gray-500">No results found.</div>
        )

      return children([...displayMode][0] as M, pageData)
    },
    dataRemote
      ? [processedData, displayMode, currentPage, mounted]
  ...
  return (
    <div className={`flex flex-col ${className ?? ''}`}>
            ...
            {filterers.map((filter) => {
              if (filter.type === 'slider')
                return (
                  <Slider
                    key={filter.name}
                    value={filterState[filter.name] as any}
                    label={filter.label}
                ...
                return (
                  <div
                    className={`${filter.className ?? ''} flex`}
                    key={filter.name}
                  >
                    <Select
                      key={filter.name}
                      selectedKeys={filterState[filter.name] as any}
                      label={filter.label}
                      radius="none"
                    ...
                    <Button
                      isIconOnly={true}
                      color="danger"
                      className="rounded-l-none rounded-r-lg h-full"
                      onClick={() =>
                        setFilterState((f) => ({
                          ...f,
                ...
                return (
                  <Switch
                    key={filter.name}
                    className={filter.className}
                    isSelected={filterState[filter.name]}
                    onValueChange={(selected) =>
                      setFilterState((f) => ({ ...f, [filter.name]: selected }))
                    }
                  >
                    {filter.label}
                  </Switch>
                ...
                return (
                  <div
                    className={`${filter.className ?? ''} flex w-full`}
                    key={filter.name}
                  >
                    <DateSelect
                      range={filterState[filter.name] as any}
                      label={filter.label}
                      radius="none"
                      className="rounded-l-lg overflow-hidden flex-grow"
                      onChange={(v) =>
                        setFilterState((f) => ({ ...f, [filter.name]: v }))
                      }
                      size="sm"
              ...
              <div className="flex gap-2 sm:w-1/3 flex-grow sm:flex-grow-0 sm:max-w-80 items-center">
                <Select
                  ...
                  onSelectionChange={(sel) =>
                    sel !== 'all' && sel.size && setPageSize(sel as Set<string>)
                  }
                >
                  {(pageSizes ?? [25, 50, 100]).map((s) => (
                    <SelectItem
                      key={s === Infinity ? 'all' : s.toString()}
                      value={s === Infinity ? 'all' : s.toString()}
                    >
                ...
                <Select
                  ...
                  selectedKeys={sorter}
                  onSelectionChange={(sel) =>
                    sel !== 'all' && sel.size && setSorter(sel as Set<string>)
                  }
                >
                  {sorters.map((s) => (
                ...
                <Tooltip
                  content={`Currently sorting ${ascending ? 'ascending' : 'descending'}`}
                >
                  <div
                  ...
                  >
                    <ArrowLongUpIcon
                      className={`w-5 transition ${ascending ? '-rotate-45' : 'rotate-45'}`}
                    />
                  </div>
                </Tooltip>
              </div>
      ...
      <div className="relative flex flex-col flex-grow items-center">
            ...
            <Dropdown>
              <DropdownTrigger>
                <Button
                  ...
                  isIconOnly={true}
                  size="sm"
                  className="p-1 bg-black/25 backdrop-blur-sm border border-gray-500/50"
                >
              ...
              <DropdownMenu
                ...
                onSelectionChange={(sel) =>
                  sel !== 'all' &&
                  sel.size &&
                  setDisplayMode(sel as Set<string>)
                }
              >
                {displayModes.map((mode) => (
                  <DropdownItem key={mode.name}>{mode.name}</DropdownItem>
                ))}
              </DropdownMenu>
            </Dropdown>
          </div>
        ...
        {renderedData === null || loadingRemoteData ? (
          <Spinner className="m-auto" />
        ) : (
          renderedData
        )}

        {totalCount > 0 && !Number.isNaN(pageSizeNum) && (
          <div className="mt-auto mb-4">
            <Pagination
              total={Math.ceil(totalCount / pageSizeNum)}
              showControls
              size={breakpoint ? 'md' : 'sm'}
              isCompact
              siblings={breakpoint ? 2 : 1}
              page={currentPage}
              initialPage={1}
...
const payloadValid = (payload: any, filterers: Filterers<any, any>) => {
  for (const p of ['sorter', 'pageSize', 'displayMode']) {
    if (p in payload && !(payload[p] instanceof Set)) return false
  }
  ...
  for (const filterer of filterers) {
    ...
    if (filterer.type === 'select' && !(data instanceof Set)) return false
    if (filterer.type === 'slider' && !Array.isArray(data)) return false
    if (
      filterer.type === 'dateSelect' &&
      data !== undefined &&
      (data.from === undefined ||
...
export const FilterSorter = <
  ...
  S extends string,
>(
  props: FilterSorterProps<D, M, N, S>,
) => {
  const pathname = usePathname()
  const localStateKey = `filter-sort-${pathname}`
    ...
    try {
      payload = JSON.parse(stored, (k, v) => {
        if (typeof v === 'object' && v !== null) {
          if ('type' in v && v.type === 'set') return new Set(v.value)
      ...
      if (!payloadValid(payload, props.filterers)) payload = {}
