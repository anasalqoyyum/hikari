...
import {
  createHikariUsersFromExistingUsers,
  userIsVisible,
  withUsersVisibleTo,
} from './user'
...
export const createHikariTeamsFromExistingTeams = async () => {
  await db.transaction().execute(async (trx) => {
    const chuniTeams = (
      await trx
        .selectFrom('chuni_profile_team as chuni')
        .leftJoin('hikari_teams as teams', 'teams.chuniTeam', 'chuni.id')
    ...
    await trx.insertInto('hikari_teams').values(insertValues).executeTakeFirst()

    await createHikariUsersFromExistingUsers(trx).catch(console.error)
...
export const getTeams = async ({
  ...
  await createHikariTeamsFromExistingTeams().catch(console.error)

  const res = await withUsersVisibleTo(user)
    .selectFrom('hikari_teams as team')
    .leftJoin('chuni_profile_team as chuni', 'team.chuniTeam', 'chuni.id')
    .leftJoin('hikari_user_ext as ext', 'ext.userId', 'team.owner')
    ...
    .select(
      ({ selectFrom, eb }) =>
        [
          ...
          'ext.uuid as ownerUuid',
          userIsVisible('owner.id').as('ownerVisible'),
          selectFrom('hikari_user_ext as uext2')
    ...
    .where(({ and, eb }) =>
      and([
        ...(uuids?.length ? [eb('team.uuid', 'in', uuids)] : [eb.lit(true)]),
      ]),
    )
    .execute()

  const userTeam = res.findIndex((t) => t.uuid === user?.team)
  if (userTeam !== -1) res.unshift(...res.splice(userTeam, 1))
...
export const syncUserTeams = async (
  user: number,
  team?: { chuniTeam: number } | null,
  transaction?: Transaction<GeneratedDB>,
) => {
  const cb = async (trx: Transaction<GeneratedDB>) => {
    if (team === undefined)
      team = await db
        .selectFrom('hikari_user_ext as ext')
        .where('ext.userId', '=', user)
        .innerJoin('hikari_teams as teams', 'teams.uuid', 'ext.team')
  ...
  if (transaction) await cb(transaction)
  else await db.transaction().execute(cb)
}
...
export const getTeamUsers = async ({
  ...
  const res = await withUsersVisibleTo(user, {
    allTeam: !!team.ownerUuid && team.ownerUuid === user?.uuid,
  })
    .selectFrom('hikari_user_ext as ext')
...
export const getTeamInviteLinks = async ({
  user,
  team,
}: {
  user?: UserPayload | null
  ...
  if (
    !hasPermission(user?.permissions, UserPermissions.OWNER) &&
    user?.uuid !== team.ownerUuid
  )
    return []

  return db
    .selectFrom('hikari_team_join_keys')
