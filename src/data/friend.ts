...
export const createHikariFriendsFromExistingFriends = async () => {
  await db
    .insertInto('hikari_user_friends')
    .columns(['user1', 'user2', 'chuniRival'])
    .expression((eb) =>
      eb
        .selectFrom('chuni_item_favorite as fav')
        .where('fav.favKind', '=', 2)
        ...
        .where(({ not, exists, selectFrom }) =>
          not(
            exists(
              selectFrom('hikari_user_friends as hikari_friends')
                .whereRef('hikari_friends.user1', '=', 'u1.id')
                .whereRef('hikari_friends.user2', '=', 'u2.id')
...
export const withChuniRivalCount = (
  builder: Kysely<GeneratedDB> | Transaction<GeneratedDB> = db,
) =>
  builder
    .with('chuni_rival_count', (db) =>
      db
        .selectFrom('chuni_item_favorite')
        .where(({ eb, lit }) => eb('favKind', '=', lit(2)))
        .groupBy('user')
        .select(
          (eb) => ['user', eb.fn.count('favId').as('rivalCount')] as const,
        ),
    )
    .with('chuni_max_rival_count', (db) =>
      db
        ...
        .select(
          ({ fn }) =>
            [
              'r1.user as user1',
              'r2.user as user2',
              fn<number>('greatest', ['r1.rivalCount', 'r2.rivalCount']).as(
                'maxRivalCount',
              ),
            ] as const,
        ),
...
export const syncUserFriends = async (
  user: number,
  builder: Kysely<GeneratedDB> | Transaction<GeneratedDB> = db,
) => {
  await builder
    .deleteFrom('chuni_item_favorite')
    .where('favKind', '=', 2)
    .where('user', '=', user)
    .where(({ not, exists, selectFrom, and, eb }) =>
      and([
        not(
          exists(
            selectFrom('hikari_user_friends as friends')
  ...
  await builder
    .insertInto('chuni_item_favorite')
    ...
    .expression((eb) =>
      eb
        ...
        .where(({ not, exists, selectFrom }) =>
          not(
            exists(
              selectFrom('chuni_item_favorite as favorite')
        ...
        .select(
          (eb) =>
            [
              'friends.user1 as user',
              'friends.user2 as favId',
              eb.lit(2).as('favKind'),
              eb
...
export const getFriends = async (user: number) => {
  return db
    .selectFrom('hikari_user_friends as friend')
