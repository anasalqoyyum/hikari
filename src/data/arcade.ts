...
const createArcadeExtIfNecessary = async () => {
  await db.transaction().execute(async (trx) => {
    const updateExtArcades = await trx
      .selectFrom('arcade')
      .leftJoin('hikari_arcade_ext as ext', 'arcade.id', 'ext.arcadeId')
    ...
    await trx
      .insertInto('hikari_arcade_ext')
      .values(
        updateExtArcades.map(({ id }) => ({
          arcadeId: id,
          uuid: sql`uuid_v4()`,
  ...
  const result = await withUsersVisibleTo(user)
    .with('owners', (eb) =>
      eb
        ...
        .innerJoin('hikari_user_ext as owner_ext', 'u.id', 'owner_ext.userId')
    ...
    .selectFrom('arcade')
    .innerJoin('hikari_arcade_ext as ext', 'arcade.id', 'ext.arcadeId')
    .leftJoin('arcade_owner', (join) =>
      join
        .onRef('arcade_owner.arcade', '=', 'arcade.id')
        .on('arcade_owner.user', '=', user?.id!),
    )
    .leftJoin('owners as owner', (join) =>
      join.onRef('owner.arcade', '=', 'arcade.id').on((eb) =>
        eb(
          'owner.id',
          '=',
          eb
    ...
    .select(({ selectFrom, or, eb }) => [
      or([
        ...
        ...(includeUnlisted
          ? [eb('ext.visibility', '=', Visibility.UNLISTED)]
          : []),
...
export const getArcadeUsers = async ({
  arcade,
  user,
  permissions,
}: ArcadeUserOpts) => {
  const res = await withUsersVisibleTo(user, {
    allArcade: hasArcadePermission(
      permissions,
      user?.permissions,
      ArcadePermissions.BOOKKEEP,
    ),
    ...
    .selectFrom('arcade_owner as o')
    .innerJoin('aime_user as u', 'u.id', 'o.user')
    .innerJoin('hikari_user_ext as uext', 'u.id', 'uext.userId')
  ...
  return res.map(({ username, visible, uuid, ...rest }) =>
    visible ? { ...rest, username, uuid } : { ...rest },
  )
}

export const getArcadeCabs = async ({
  arcade,
  user,
  permissions,
}: ArcadeUserOpts) => {
  return db
    ...
    .select([
      ...
      ...(hasArcadePermission(
        ...
        ? (['board', 'serial'] as const)
        : []),
    ])
    .execute()
}

export const getArcadeInviteLinks = async ({
  arcade,
  user,
  permissions,
}: ArcadeUserOpts) => {
  if (
    !hasPermission(permissions, ArcadePermissions.OWNER) &&
    !hasPermission(user?.permissions, UserPermissions.OWNER)
  )
    return []

  return db
    .selectFrom('hikari_arcade_join_keys')
    .where('arcadeId', '=', arcade)
    .selectAll()
    .execute()
}

export const getArcadePermissions = async (user: UserPayload, arcade: number) =>
  (
    await db
      .selectFrom('arcade_owner as o')
      .where('o.arcade', '=', arcade)
...
export const countryValidator = (val: string | null | undefined) => {
  if (!COUNTRY_CODES.has(val!)) throw new Error('Invalid country')
}
...
export type ArcadeLink = Awaited<
  ReturnType<typeof getArcadeInviteLinks>
>[number]
